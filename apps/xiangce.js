(() => {
  ...
  const pickerSheet = document.createElement('aside');
  ...
  pickerSheet.addEventListener('click', event => {
    ...
    if (item) {
      ...
      const id = item.dataset.albumPickerMultiItem;
      if (pickerMultiSelected.has(id)) pickerMultiSelected.delete(id);
      else if (pickerMultiSelected.size < pickerMultiMax) pickerMultiSelected.add(id);
      renderMultiPickerSheet();
      return;
    }
    const done = event.target.closest('[data-album-picker-multi-done]');
    if (done) {
      event.preventDefault();
      event.stopImmediatePropagation();
      const selected = [...pickerMultiSelected].map(id => state.items.find(entry => entry.id === id)).filter(Boolean);
      const callback = pickerCallback;
      closePickerSheet();
      callback?.(selected.map(entry => entry.url), selected);
    }
  });
  ...
  function urlName(value) {
    try {
      ...
      return String(filename || parsed.hostname || 'URL 图片').slice(0, 120);
    } catch { return 'URL 图片'; }
  }
  ...
  function archiveUrl(value, source = '其他 App') {
    ...
    if (existing) return existing;
    const item = {
      id:uid(), url, name:urlName(url), source,
      createdAt:Date.now(), size:0, type:'image/*', width:0, height:0,
      fingerprint:`url:${url}`, hosting:'public'
    };
    state.items.unshift(item);
    saveState();
    if (app.classList.contains('is-open')) render();
  ...
  async function deleteItems(ids) {
    ...
    const items = state.items.filter(entry => idSet.has(entry.id));
    if (!items.length || !window.confirm(`确定从相册删除选中的 ${items.length} 张图片吗？其他 App 正在使用的图片不会被清除。`)) return;
    const references = items.map(item => ({ item, referencedElsewhere:Object.keys(localStorage).some(key => key !== storageKey && (localStorage.getItem(key) || '').includes(item.url)) }));
    ...
    render();
    showToast(`已删除 ${items.length} 张图片`);
  }
  ...
  document.addEventListener('click', event => {
    ...
    if (pickerClose) { closePickerSheet(); return; }
    const pickerLibrary = event.target.closest('[data-album-picker-open-library]');
    if (pickerLibrary) { closePickerSheet(); document.querySelector('[data-app-key="xiangce"]')?.click(); return; }
    const pickerItem = event.target.closest('[data-album-picker-item]');
    if (pickerItem) {
      const item = state.items.find(entry => entry.id === pickerItem.dataset.albumPickerItem);
      const callback = pickerCallback;
      closePickerSheet();
      callback?.(item?.url || '', item || null);
      return;
    }
    ...
    if (open) {
      if (pickerMode) {
        const item = state.items.find(entry => entry.id === open.dataset.albumOpen);
        const callback = pickerCallback;
        pickerMode = false;
        pickerCallback = null;
        selecting = false;
  ...
  document.addEventListener('keydown', event => {
    ...
    else if (selecting) { selecting = false; selectedIds.clear(); render(); }
    else { pickerMode = false; pickerCallback = null; app.classList.remove('is-open'); app.setAttribute('aria-hidden', 'true'); }
  });
