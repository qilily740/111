(() => {
  ...
  function memoryOwnerId(roleId, chat, role) { return role?.isGroup && role.taRoleId ? role.taRoleId : chat?.isTaGroup && chat.taRoleId ? chat.taRoleId : roleId; }
  function memoryOwnerRole(roleId, chat, role) { const ownerId = memoryOwnerId(roleId, chat, role); return readJSON(chatKey, {}).contacts?.find(item => item.id === ownerId) || role; }
  ...
  async function prepareContext({roleId,chat,query,profileId=chat?.profileId||''}) {
    const settings=settingsFor(chat), reserve=Math.min(2000,Math.floor(settings.contextTokenBudget*.2)), scopedRoleId=memoryOwnerId(roleId,chat);
    ...
    const library=readLibrary(), migrated=migrateRoleScope(library,scopedRoleId,profileId), key=scopeKey(scopedRoleId,profileId);
    const all=library.entries.filter(item=>entryMatchesScope(item,scopedRoleId,profileId)&&(item.level==='long'||item.type==='chat'||item.type==='offline')).sort((a,b)=>Number(b.createdAt||0)-Number(a.createdAt||0));
  ...
  async function processAvailable(options={}) {
    const requestedRoleId=options.roleId, chat=options.chat||readJSON(chatKey,{}).chats?.[requestedRoleId], profileId=options.profileId??chat?.profileId??'', role=options.role||readJSON(chatKey,{}).contacts?.find(item=>item.id===requestedRoleId), roleId=memoryOwnerId(requestedRoleId,chat,role), scopedRole=memoryOwnerRole(requestedRoleId,chat,role); if(!requestedRoleId)return{created:0,coreUpdated:false,pendingRounds:0}; const key=scopeKey(roleId,profileId); if(processing.has(key))return processing.get(key);
    const task=(async()=>{const settings=settingsFor(chat);if(!chat||!settings.enabled||(!options.force&&!settings.autoSummary))return{created:0,coreUpdated:false,pendingRounds:0};const library=readLibrary();if(migrateRoleScope(library,roleId,profileId))saveLibrary(library);const meta=library.roleMeta[key]||=( {roleId,profileId} ),messages=Array.isArray(chat.messages)?chat.messages:[];let startIndex=0;if(meta.lastSummarizedMessageId){const found=messages.findIndex(item=>item.id===meta.lastSummarizedMessageId);if(found>=0)startIndex=found+1;else meta.lastSummarizedMessageId='';}const rounds=completedRounds(messages,startIndex,profileId),maxBatches=clamp(options.maxBatches,1,50,options.force?20:3);let created=0,consumed=0;while(rounds.length-consumed>=settings.summaryEveryRounds&&consumed/settings.summaryEveryRounds<maxBatches){const batch=rounds.slice(consumed,consumed+settings.summaryEveryRounds),entry=await summarizeBatch({...options,roleId,role:scopedRole,profileId,rounds:batch,settings,library});if(entry)created+=1;const last=batch.at(-1)?.messages.at(-1);if(last?.id)meta.lastSummarizedMessageId=last.id;meta.summarizedRounds=Number(meta.summarizedRounds||0)+batch.length;consumed+=batch.length;saveLibrary(library);}const hasEntries=library.entries.some(item=>entryMatchesScope(item,roleId,profileId));const coreUpdated=(created||options.force)&&hasEntries?await updateCore({...options,roleId,role:scopedRole,profileId,settings,library,force:Boolean(options.force)}):false;const embedded=options.force?await backfillEmbeddings(roleId,profileId,settings,library):0;if(coreUpdated||embedded)saveLibrary(library);return{created,coreUpdated,pendingRounds:Math.max(0,rounds.length-consumed)};})().finally(()=>processing.delete(key));processing.set(key,task);return task;
  }
