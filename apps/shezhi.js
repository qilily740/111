(() => {
  ...
  const defaultSettings = { api: { endpoint: '', key: '', models: [], availableModels: [], selected: [], assignments: {}, profiles: [], activeProfileId: '' }, vectorApi: { endpoint: '', key: '', model: '', availableModels: [], profiles: [], activeProfileId: '' }, notifications: { enabled: true }, chatPrompt: { custom: '' }, keepAlive: { enabled: false } };
...
按照本提示词中定义的格式输出。除非本提示词明确要求，否则不要输出 JSON、XML、Markdown 代码块、动作名称、字段解释或系统说明。保持回复像角色本人真正发出的消息。`;
  ...
  app.innerHTML = `<div class="settings-page"><header class="settings-header"><div><span class="settings-kicker">SYSTEM PREFERENCES</span><h1>设置</h1><p>把连接、数据与空间，收纳在一处。</p></div><button class="settings-close" data-settings-close type="button">×</button></header><main class="settings-main"><section class="settings-section settings-api-section"><div class="settings-section-head"><div><span class="settings-eyebrow">INTELLIGENCE</span><h2>API 接入</h2><p>连接兼容 OpenAI API 的模型服务，并为不同功能分配模型。</p></div><span class="settings-status" id="settingsApiStatus">未连接</span></div><div class="settings-profile-bar"><div class="settings-profile-picker"><button class="settings-profile-trigger" data-settings-profile-toggle type="button"><span id="settingsProfileCurrent">选择已保存的 API 配置</span><i>⌄</i></button><div class="settings-profile-menu" id="settingsProfileMenu" hidden></div></div><button data-settings-new-profile type="button">＋ 新配置</button></div><div class="settings-api-form"><label>配置名称<input id="settingsProfileName" type="text" placeholder="例如：主力模型服务"></label><label>接口地址<input id="settingsEndpoint" type="url" placeholder="https://api.example.com/v1"></label><label>API Key<input id="settingsApiKey" type="password" placeholder="仅保存在本机浏览器"></label><button class="settings-primary" data-settings-fetch type="button">拉取模型</button></div><div class="settings-model-panel"><div class="settings-subhead"><b>保留模型</b><small id="settingsModelHint">还没有拉取模型</small></div><div class="settings-model-list" id="settingsModelList"><div class="settings-empty">填写接口地址后拉取模型</div></div></div><div class="settings-assignment"><div class="settings-subhead"><b>功能分配</b><small>每个功能可使用不同模型</small></div><div class="settings-assignment-grid" id="settingsAssignments"></div></div><button class="settings-save-api" data-settings-save-api type="button">保存当前配置</button></section><section class="settings-section settings-storage-section"><div class="settings-section-head"><div><span class="settings-eyebrow">STORAGE</span><h2>存储空间分析</h2><p>按 App 汇总本地数据。</p></div><span class="settings-storage-total" id="settingsStorageTotal">0 KB</span></div><div class="settings-storage-list" id="settingsStorageList"></div></section><section class="settings-section settings-data-section"><div class="settings-section-head"><div><span class="settings-eyebrow">YOUR DATA</span><h2>数据管理</h2><p>备份或迁移所有 App 的本地数据。</p></div></div><div class="settings-data-actions"><button data-settings-export type="button">导出全部数据 <small>JSON</small></button><label class="settings-import-label">导入全部数据 <small>JSON</small><input id="settingsImport" type="file" accept="application/json,.json"></label></div></section><section class="settings-section settings-danger-section"><div class="settings-section-head"><div><span class="settings-eyebrow">RESET</span><h2>恢复初始设置</h2><p>清空所有 App 数据、壁纸、模型与页面修改，恢复为第一次打开的状态。</p></div></div><button class="settings-danger" data-settings-reset type="button">清空缓存并恢复初始设置</button></section></main></div>`;
  ...
  function setupCollapsibleSections() {
    const sections = app.querySelectorAll('.settings-api-section, .settings-image-api-section, .settings-vector-section, .settings-prompt-section, .settings-floating-section, .settings-storage-section');
    sections.forEach(section => {
      const header = section.querySelector('.settings-section-head');
      if (!header || header.dataset.settingsCollapseReady === 'true') return;
      const key = section.classList.contains('settings-api-section') ? 'api' : section.classList.contains('settings-image-api-section') ? 'image' : section.classList.contains('settings-vector-section') ? 'vector' : section.classList.contains('settings-floating-section') ? 'floating' : section.classList.contains('settings-storage-section') ? 'storage' : 'prompt';
  ...
  function readSettings() { try { const saved = JSON.parse(localStorage.getItem(storageKey) || '{}'); const result = { ...clone(defaultSettings), ...saved, api: { ...clone(defaultSettings.api), ...(saved.api || {}) }, vectorApi: { ...clone(defaultSettings.vectorApi), ...(saved.vectorApi || {}) }, notifications: { ...clone(defaultSettings.notifications), ...(saved.notifications || {}) }, chatPrompt: { ...clone(defaultSettings.chatPrompt), ...(saved.chatPrompt || {}) }, keepAlive: { ...clone(defaultSettings.keepAlive), ...(saved.keepAlive || {}) } }; if (!Array.isArray(result.api.profiles)) result.api.profiles = []; if (!result.api.profiles.length && result.api.endpoint) { result.api.profiles = [{ id: `profile-${Date.now()}`, name: '默认 API', endpoint: result.api.endpoint, key: result.api.key, models: result.api.models || [], availableModels: result.api.availableModels || [], selected: result.api.selected || [], assignments: result.api.assignments || {} }]; result.api.activeProfileId = result.api.profiles[0].id; } if (!Array.isArray(result.vectorApi.profiles)) result.vectorApi.profiles = []; if (!result.vectorApi.profiles.length && result.vectorApi.endpoint) { const id = `vector-profile-${Date.now()}`; result.vectorApi.profiles = [{ id, name: result.vectorApi.endpoint, endpoint: result.vectorApi.endpoint, key: result.vectorApi.key, model: result.vectorApi.model, availableModels: result.vectorApi.availableModels || [] }]; result.vectorApi.activeProfileId = id; } result.vectorApi.activeProfileId = result.vectorApi.profiles.some(profile => profile.id === result.vectorApi.activeProfileId) ? result.vectorApi.activeProfileId : (result.vectorApi.profiles[0]?.id || ''); return result; }
  ...
  function render() { models = settings.api.availableModels?.length ? settings.api.availableModels : (settings.api.models || []); const profiles = settings.api.profiles || []; const active = profiles.find(profile => profile.id === settings.api.activeProfileId); const current = document.querySelector('#settingsProfileCurrent'); const menu = document.querySelector('#settingsProfileMenu'); if (current) { current.textContent = active ? `${active.name} · ${active.endpoint}` : (profiles.length ? '选择已保存的 API 配置' : '暂无已保存的 API 配置'); current.title = active ? active.endpoint || '' : ''; } if (menu) { menu.innerHTML = profiles.length ? profiles.map(profile => `<div class="settings-profile-item-row"><button class="settings-profile-item ${profile.id === settings.api.activeProfileId ? 'is-active' : ''}" data-settings-profile-select="${esc(profile.id)}" type="button"><span><b>${esc(profile.name)}</b><small>${esc(profile.endpoint)}</small></span></button><button class="settings-profile-delete" data-settings-profile-delete="${esc(profile.id)}" type="button" aria-label="删除 ${esc(profile.name)}">删除</button></div>`).join('') : '<div class="settings-profile-empty">暂无已保存的 API 配置</div>'; menu.hidden = !profileMenuOpen; } document.querySelector('#settingsProfileName').value = active?.name || ''; document.querySelector('#settingsEndpoint').value = settings.api.endpoint || ''; document.querySelector('#settingsApiKey').value = settings.api.key || ''; renderModels(); renderStorage(); }
  ...
  function switchProfile(id) { const profile = settings.api.profiles?.find(item => item.id === id); profileMenuOpen = false; if (!profile) { render(); return; } settings.api = { ...settings.api, ...clone(profile), profiles: settings.api.profiles, activeProfileId: profile.id }; saveSettings(); render(); setStatus(`已切换：${profile.name}`); }
  function deleteProfile(id) { const profile = settings.api.profiles?.find(item => item.id === id); if (!profile) return; if (!window.confirm(`确定删除 API 配置“${profile.name}”吗？`)) return; const draft = { name: document.querySelector('#settingsProfileName')?.value || '', endpoint: document.querySelector('#settingsEndpoint')?.value || '', key: document.querySelector('#settingsApiKey')?.value || '' }; const profiles = (settings.api.profiles || []).filter(item => item.id !== id); const wasActive = settings.api.activeProfileId === id; if (wasActive) { const next = profiles[0]; settings.api = next ? { ...settings.api, ...clone(next), profiles, activeProfileId: next.id } : { ...settings.api, endpoint:'', key:'', models:[], availableModels:[], selected:[], assignments:{}, profiles:[], activeProfileId:'' }; models = next ? (next.availableModels?.length ? next.availableModels : (next.models || [])) : []; } else settings.api.profiles = profiles; profileMenuOpen = false; saveSettings(); render(); if (!wasActive) { document.querySelector('#settingsProfileName').value = draft.name; document.querySelector('#settingsEndpoint').value = draft.endpoint; document.querySelector('#settingsApiKey').value = draft.key; } setStatus(wasActive ? (profiles.length ? `已切换：${profiles[0].name}` : '已删除 API 配置') : `已删除：${profile.name}`); }
  ...
  let vectorProfileMenuOpen = false;
  let vectorEditingProfileId = null;
  function vectorApiFromForm() { const value = { endpoint: document.querySelector('#settingsVectorEndpoint')?.value.trim().replace(/\/$/, '') || '', key: document.querySelector('#settingsVectorApiKey')?.value.trim() || '', model: document.querySelector('#settingsVectorModel')?.value.trim() || '', availableModels: settings.vectorApi?.availableModels || [] }; if (vectorEditingProfileId !== null) value.activeProfileId = vectorEditingProfileId; return value; }
  function vectorProfileValues(config) { return { endpoint: config.endpoint || '', key: config.key || '', model: config.model || '', availableModels: Array.isArray(config.availableModels) ? config.availableModels : [] }; }
  ...
  function renderVectorProfiles(current) { const profiles = current.profiles || []; const active = profiles.find(profile => profile.id === current.activeProfileId); const label = document.querySelector('#settingsVectorProfileCurrent'); const menu = document.querySelector('#settingsVectorProfileMenu'); if (label) { label.textContent = active ? `${active.name} · ${active.endpoint}` : (profiles.length ? '选择已保存的向量配置' : '暂无已保存的向量配置'); label.title = active ? active.endpoint || '' : ''; } if (menu) { menu.innerHTML = profiles.length ? profiles.map(profile => `<div class="settings-profile-item-row"><button class="settings-profile-item ${profile.id === current.activeProfileId ? 'is-active' : ''}" data-settings-vector-profile-select="${esc(profile.id)}" type="button"><span><b>${esc(profile.name)}</b><small>${esc(profile.endpoint)}</small></span></button><button class="settings-profile-delete" data-settings-vector-profile-delete="${esc(profile.id)}" type="button" aria-label="删除 ${esc(profile.name)}">删除</button></div>`).join('') : '<div class="settings-profile-empty">暂无已保存的向量配置</div>'; menu.hidden = !vectorProfileMenuOpen; } }
  function renderVectorSettings() { const current = settings.vectorApi || defaultSettings.vectorApi; vectorEditingProfileId = current.activeProfileId || ''; const endpoint = document.querySelector('#settingsVectorEndpoint'); const key = document.querySelector('#settingsVectorApiKey'); const model = document.querySelector('#settingsVectorModel'); const list = document.querySelector('#settingsVectorModels'); if (!endpoint || !key || !model || !list) return; renderVectorProfiles(current); endpoint.value = current.endpoint || ''; key.value = current.key || ''; model.value = current.model || ''; list.innerHTML = (current.availableModels || []).map(item => `<option value="${esc(item)}"></option>`).join(''); setVectorStatus(current.endpoint && current.model ? '已配置' : '未配置', current.endpoint && current.model ? 'ready' : ''); }
  ...
  function saveVectorSettings(showStatus = true) { const value = vectorApiFromForm(); if (!value.endpoint || !value.model) { setVectorStatus('请填写接口和模型', 'error'); return false; } const current = settings.vectorApi || defaultSettings.vectorApi; const profiles = Array.isArray(current.profiles) ? [...current.profiles] : []; const id = vectorEditingProfileId || `vector-profile-${Date.now()}`; const previous = profiles.find(profile => profile.id === id); const profile = { id, name: previous?.name || value.endpoint, ...vectorProfileValues(value) }; const index = profiles.findIndex(item => item.id === id); if (index >= 0) profiles[index] = profile; else profiles.push(profile); settings.vectorApi = { ...current, ...value, profiles, activeProfileId: id }; vectorEditingProfileId = id; vectorProfileMenuOpen = false; saveSettings(); renderVectorSettings(); if (showStatus) setVectorStatus('已保存并关联记忆库', 'ready'); return true; }
  function switchVectorProfile(id) { const current = settings.vectorApi || defaultSettings.vectorApi; const profile = (current.profiles || []).find(item => item.id === id); vectorProfileMenuOpen = false; if (!profile) return renderVectorSettings(); vectorEditingProfileId = profile.id; settings.vectorApi = { ...current, ...vectorProfileValues(profile), activeProfileId: profile.id }; saveSettings(); renderVectorSettings(); setVectorStatus(`已切换：${profile.name}`, 'ready'); }
  function deleteVectorProfile(id) { const current = settings.vectorApi || defaultSettings.vectorApi; const profile = (current.profiles || []).find(item => item.id === id); if (!profile || !window.confirm(`确定删除向量配置“${profile.name}”吗？`)) return; const profiles = current.profiles.filter(item => item.id !== id); const wasActive = current.activeProfileId === id; const draft = vectorApiFromForm(); if (wasActive) { const next = profiles[0]; vectorEditingProfileId = next?.id || ''; settings.vectorApi = next ? { ...current, ...vectorProfileValues(next), profiles, activeProfileId: next.id } : { ...current, ...vectorProfileValues(defaultSettings.vectorApi), profiles: [], activeProfileId: '' }; } else settings.vectorApi = { ...current, profiles }; vectorProfileMenuOpen = false; saveSettings(); renderVectorSettings(); if (!wasActive) { document.querySelector('#settingsVectorEndpoint').value = draft.endpoint; document.querySelector('#settingsVectorApiKey').value = draft.key; document.querySelector('#settingsVectorModel').value = draft.model; } setVectorStatus(wasActive ? (profiles.length ? `已切换：${profiles[0].name}` : '已删除向量配置') : `已删除：${profile.name}`, 'ready'); }
  async function fetchVectorModels() { const value = vectorApiFromForm(); if (!value.endpoint) return setVectorStatus('请先填写接口地址', 'error'); const button = document.querySelector('[data-settings-vector-fetch]'); button.disabled = true; button.textContent = '读取中…'; setVectorStatus('正在读取', 'busy'); try { const response = await fetch(`${value.endpoint}/models`, { headers: vectorHeaders(value.key) }); if (!response.ok) throw new Error(`HTTP ${response.status}`); const payload = await response.json(); const fetched = Array.isArray(payload.data) ? payload.data.map(item => typeof item === 'string' ? item : item.id).filter(Boolean) : []; if (!fetched.length) throw new Error('没有找到模型，可直接手动填写'); const vectorFirst = [...new Set(fetched)].sort((a, b) => Number(/embed|bge|e5|gte|vector/i.test(b)) - Number(/embed|bge|e5|gte|vector/i.test(a)) || a.localeCompare(b)); settings.vectorApi = { ...settings.vectorApi, ...value, availableModels: vectorFirst }; const activeIndex = (settings.vectorApi.profiles || []).findIndex(profile => profile.id === settings.vectorApi.activeProfileId); if (activeIndex >= 0) settings.vectorApi.profiles[activeIndex] = { ...settings.vectorApi.profiles[activeIndex], ...vectorProfileValues(settings.vectorApi) }; const list = document.querySelector('#settingsVectorModels'); if (list) list.innerHTML = vectorFirst.map(item => `<option value="${esc(item)}"></option>`).join(''); const input = document.querySelector('#settingsVectorModel'); if (input && !input.value.trim()) input.value = vectorFirst[0]; setVectorStatus(`已找到 ${vectorFirst.length} 个模型`, 'ready'); } catch (error) { setVectorStatus(`读取失败：${error.message}`, 'error'); } finally { button.disabled = false; button.textContent = '读取模型'; } }
  async function testVectorConnection() { const value = vectorApiFromForm(); if (!value.endpoint || !value.model) return setVectorStatus('请填写接口和模型', 'error'); const button = document.querySelector('[data-settings-vector-test]'); button.disabled = true; button.textContent = '测试中…'; setVectorStatus('正在生成测试向量', 'busy'); try { const response = await fetch(`${value.endpoint}/embeddings`, { method:'POST', headers:vectorHeaders(value.key), body:JSON.stringify({ model:value.model, input:'理想机记忆连接测试' }) }); if (!response.ok) throw new Error(`HTTP ${response.status}`); const payload = await response.json(); const vector = payload.data?.[0]?.embedding; if (!Array.isArray(vector) || !vector.length) throw new Error('接口没有返回向量数据'); settings.vectorApi = { ...settings.vectorApi, ...value }; saveVectorSettings(false); setVectorStatus(`连接成功 · ${vector.length} 维`, 'ready'); } catch (error) { setVectorStatus(`连接失败：${error.message}`, 'error'); } finally { button.disabled = false; button.textContent = '测试连接'; } }
