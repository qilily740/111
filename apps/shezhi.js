...
按照本提示词中定义的格式输出。除非本提示词明确要求，否则不要输出 JSON、XML、Markdown 代码块、动作名称、字段解释或系统说明。保持回复像角色本人真正发出的消息。`;
  ...
  app.querySelector('.settings-storage-section').insertAdjacentHTML('beforebegin', `<section class="settings-section settings-notification-section"><div class="settings-section-head"><div><span class="settings-eyebrow">NOTIFICATIONS</span><h2>角色消息通知</h2><p>离开当前聊天后，角色回复会从屏幕顶部弹出。</p></div></div><div class="settings-notification-row"><div><b>顶部消息弹窗</b><small>显示角色头像、网名和回复内容</small></div><label class="settings-notification-switch"><input type="checkbox" data-settings-notification-toggle><i></i></label></div><button class="settings-notification-test" data-settings-notification-test type="button">测试通知</button></section>`);
