...
按照本提示词中定义的格式输出。除非本提示词明确要求，否则不要输出 JSON、XML、Markdown 代码块、动作名称、字段解释或系统说明。保持回复像角色本人真正发出的消息。`;
  ...
  app.querySelector('.settings-storage-section').insertAdjacentHTML('beforebegin', `<section class="settings-section settings-keepalive-section"><div class="settings-section-head"><div><span class="settings-eyebrow">BACKGROUND AUDIO</span><h2>音频保活</h2><p>保持页面活跃；音乐播放时会自动让位。</p></div><span class="settings-status" id="settingsKeepAliveStatus">已关闭</span></div><div class="settings-notification-row"><div><b>开启保活</b><small>使用不可听见的低占用音频状态</small></div><label class="settings-notification-switch"><input type="checkbox" data-settings-keepalive-toggle><i></i></label></div></section>`);
  ...
  app.querySelector('.settings-storage-section').insertAdjacentHTML('beforebegin', `<section class="settings-section settings-notification-section"><div class="settings-section-head"><div><span class="settings-eyebrow">NOTIFICATIONS</span><h2>角色消息通知</h2><p>离开当前聊天后，角色名和实际消息会显示在系统通知中心。</p></div></div><div class="settings-notification-row"><div><b>系统消息通知</b><small>使用 Ideal 图标，逐条显示真实角色消息</small></div><label class="settings-notification-switch"><input type="checkbox" data-settings-notification-toggle><i></i></label></div><button class="settings-notification-test" data-settings-notification-test type="button">允许并测试 Ideal 通知</button></section>`);
  ...
  window.IdealMachineNotifications = { show: showMessageNotification, showSystem: showSystemNotification, requestPermission: requestSystemNotificationPermission, hide: hideMessageNotification, enabled: notificationsEnabled };
  ...
  async function startKeepAliveAudio() { if (musicIsPlaying()) return; const AudioContextClass = window.AudioContext || window.webkitAudioContext; if (!AudioContextClass) { keepAliveState = 'unsupported'; return; } if (!keepAliveContext) { keepAliveContext = new AudioContextClass(); keepAliveGain = keepAliveContext.createGain(); keepAliveGain.gain.value = 0.000001; keepAliveGain.connect(keepAliveContext.destination); keepAliveOscillator = keepAliveContext.createOscillator(); keepAliveOscillator.type = 'sine'; keepAliveOscillator.frequency.value = 18; keepAliveOscillator.connect(keepAliveGain); keepAliveOscillator.start(); } if (keepAliveContext.state === 'suspended') await keepAliveContext.resume(); keepAliveState = 'running'; }
  async function pauseKeepAliveAudio() { if (keepAliveContext?.state === 'running') await keepAliveContext.suspend(); if (settings.keepAlive?.enabled) keepAliveState = musicIsPlaying() ? 'music' : 'paused'; }
  async function syncKeepAlive() { if (settings.keepAlive?.enabled !== true) { await pauseKeepAliveAudio(); keepAliveState = 'off'; renderKeepAliveSettings(); return; } if (musicIsPlaying()) { await pauseKeepAliveAudio(); keepAliveState = 'music'; } else { try { await startKeepAliveAudio(); } catch { keepAliveState = 'blocked'; } } renderKeepAliveSettings(); }
  function renderKeepAliveSettings() { const toggle = document.querySelector('[data-settings-keepalive-toggle]'); const status = document.querySelector('#settingsKeepAliveStatus'); if (toggle) toggle.checked = settings.keepAlive?.enabled === true; if (!status) return; status.textContent = settings.keepAlive?.enabled !== true ? '已关闭' : keepAliveState === 'music' ? '音乐播放中，已让位' : keepAliveState === 'running' ? '运行中' : keepAliveState === 'unsupported' ? '设备不支持' : keepAliveState === 'blocked' ? '等待音频权限' : '已开启'; status.dataset.state = settings.keepAlive?.enabled === true && keepAliveState !== 'blocked' ? 'ready' : ''; }
  window.IdealMachineKeepAlive = { sync: syncKeepAlive, setEnabled(enabled) { settings.keepAlive = { ...(settings.keepAlive || {}), enabled:Boolean(enabled) }; saveSettings(); syncKeepAlive(); }, pauseForMusic: pauseKeepAliveAudio, isEnabled:() => settings.keepAlive?.enabled === true };
  ...
  document.addEventListener('click', event => { if (!app.classList.contains('is-open') || !event.target.closest('[data-settings-notification-test]')) return; requestSystemNotificationPermission().then(async () => { const shown = await showSystemNotification({ name:'Ideal', message:'这是一条 Ideal 系统通知测试。', messageId:`test-${Date.now()}` }, true); if (!shown) window.alert('系统通知发送失败，请检查浏览器或手机系统是否允许 Ideal 显示通知。'); }).catch(error => window.alert(error.message)); });
