...
按照本提示词中定义的格式输出。除非本提示词明确要求，否则不要输出 JSON、XML、Markdown 代码块、动作名称、字段解释或系统说明。保持回复像角色本人真正发出的消息。`;
  ...
  function ensureKeepAliveAudio() { if (keepAliveAudio) return keepAliveAudio; keepAliveAudio = document.createElement('audio'); keepAliveAudio.className = 'ideal-keepalive-audio'; keepAliveAudio.preload = 'auto'; keepAliveAudio.loop = true; keepAliveAudio.playsInline = true; keepAliveAudio.volume = 0.01; keepAliveAudio.setAttribute('aria-hidden', 'true'); keepAliveAudioUrl = createKeepAliveAudioUrl(); keepAliveAudio.src = keepAliveAudioUrl; keepAliveAudio.addEventListener('play', () => { updateKeepAliveMediaSession(true); }); keepAliveAudio.addEventListener('pause', () => { if (settings.keepAlive?.enabled && !musicIsPlaying() && keepAliveState === 'running') { keepAliveState = 'paused'; renderKeepAliveSettings(); } }); document.body.appendChild(keepAliveAudio); return keepAliveAudio; }
