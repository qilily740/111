(() => {
  ...
  const defaultSettings = { api: { endpoint: '', key: '', models: [], availableModels: [], selected: [], assignments: {}, profiles: [], activeProfileId: '' }, vectorApi: { endpoint: '', key: '', model: '', availableModels: [] }, notifications: { enabled: true }, chatPrompt: { custom: '' } };
...
按照本提示词中定义的格式输出。除非本提示词明确要求，否则不要输出 JSON、XML、Markdown 代码块、动作名称、字段解释或系统说明。保持回复像角色本人真正发出的消息。`;
  ...
  function readSettings() { try { const saved = JSON.parse(localStorage.getItem(storageKey) || '{}'); const result = { ...clone(defaultSettings), ...saved, api: { ...clone(defaultSettings.api), ...(saved.api || {}) }, vectorApi: { ...clone(defaultSettings.vectorApi), ...(saved.vectorApi || {}) }, notifications: { ...clone(defaultSettings.notifications), ...(saved.notifications || {}) }, chatPrompt: { ...clone(defaultSettings.chatPrompt), ...(saved.chatPrompt || {}) } }; if (!result.api.profiles.length && result.api.endpoint) { result.api.profiles = [{ id: `profile-${Date.now()}`, name: '默认 API', endpoint: result.api.endpoint, key: result.api.key, models: result.api.models || [], selected: result.api.selected || [], assignments: result.api.assignments || {} }]; result.api.activeProfileId = result.api.profiles[0].id; } return result; }
  ...
  render = function() { baseSettingsRender(); renderApiStatus(); renderVectorSettings(); renderNotificationSettings(); renderPromptSettings(); };
