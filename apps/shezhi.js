...
按照本提示词中定义的格式输出。除非本提示词明确要求，否则不要输出 JSON、XML、Markdown 代码块、动作名称、字段解释或系统说明。保持回复像角色本人真正发出的消息。`;
  ...
  function configuredUserAvatar() { try { const desktop = JSON.parse(localStorage.getItem('ideal-machine-desktop') || '{}'); return String(desktop?.['user-avatar'] || ''); } catch { return ''; } }
  ...
  function showInternalNotification({ contactId = '', name = '新消息', message = '' } = {}) { notificationTarget = contactId; const avatarNode = notificationBanner.querySelector('.ideal-message-notification-avatar'); const nameNode = notificationBanner.querySelector('b'); const messageNode = notificationBanner.querySelector('small'); const token = ++notificationAvatarToken; const applyAvatar = value => { if (token !== notificationAvatarToken || !avatarNode.isConnected) return; avatarNode.innerHTML = ''; if (value) { const image = document.createElement('img'); image.src = value; image.alt = ''; avatarNode.appendChild(image); } else avatarNode.textContent = String(name || '消').slice(0, 1); }; const source = configuredUserAvatar(); applyAvatar(source.startsWith('idb:image:') ? '' : source); if (source.startsWith('idb:image:')) window.IdealMachineGetImage?.(source).then(applyAvatar).catch(() => {}); nameNode.textContent = name || '新消息'; messageNode.textContent = message || '收到一条新消息'; notificationBanner.classList.remove('is-visible'); requestAnimationFrame(() => notificationBanner.classList.add('is-visible')); clearTimeout(notificationTimer); notificationTimer = setTimeout(hideMessageNotification, 3000); return true; }
