(() => {
  ...
  const refreshSelectedAppGenerated = refreshSelectedApp;
  refreshSelectedApp = async function(owner, key, options) { const before = key === 'music' ? snapshot(owner) : null; const result = await refreshSelectedAppGenerated(owner, key, options); if (key === 'music') { const all = read('ideal-machine-ta-snapshots', {}); const latest = all[owner.id] || {}; latest.music = mergeRoleMusic(before?.music, latest.music); all[owner.id] = latest; localStorage.setItem('ideal-machine-ta-snapshots', JSON.stringify(all)); render(); } return result; };
...
可用 NPC：${existing.map(item => `${item.name}（${item.identity || item.reason || '关系未知'}）`).join('；') || '根据角色设定选择确实有关的人物'}`;
  ...
  function roleMusic(owner) {
    ...
    return `<section class="ta-recent-playlist"><header><div><small>RECENTLY PLAYED</small><h2>${esc(owner.nickname || owner.name)} 最近在听</h2></div><span>${rows.length} 首</span></header><div>${rows.map((item, index) => { const count = String(item.playCount || item.plays || item.count || '1次'); return `<button type="button" data-ta-detail="music" data-ta-detail-index="${index}"><em>${index + 1}</em><i>${builtinAppIcon('yinyue', icon('music'))}</i><span><b>${esc(item.title || item.name || '未知歌曲')}</b><small>${esc(item.artist || '未知歌手')}</small></span><strong>听了 ${esc(/次$/.test(count) ? count : `${count}次`)}</strong><u>›</u></button>`; }).join('')}</div></section>`;
  }
  ...
  function restoreLegacyStickerUrl(value) { const source=String(value||'').trim(); const match=source.match(/^\.?\/?assets\/stickers\/postimg\/([^/]+?)-([^/?#]+)$/i); return match ? `https://i.postimg.cc/${match[1]}/${match[2]}` : source; }
  async function reverseVisualUrl(value) { let url=restoreLegacyStickerUrl(value||'');try{if(String(url).startsWith('idb:image:')&&window.IdealMachineGetImage)url=await window.IdealMachineGetImage(url);}catch{url='';}return url; }
  ...
  let reversePreferenceUsed = false;
  ...
  function reversePreferredMatch(item) { return Array.isArray(item)&&reversePreferredApp&&(item[0]===reversePreferredApp||reverseAppMeta(item[0])[0]===reversePreferredApp||reverseAppMeta(item[0])[1]===reversePreferredApp); }
  function reverseShuffle(items) { const result=[...items];for(let index=result.length-1;index>0;index-=1){const swap=Math.floor(Math.random()*(index+1));[result[index],result[swap]]=[result[swap],result[index]];}return result; }
  ...
  function rawTakeoverBubble(value) { return String(value||'').replace(/[“”"]/g,'').replace(/\\n/g,' ').replace(/\s+/g,' ').trim(); }
  ...
  function incompleteTakeoverBubble(value,minLength=4) {
    const raw=rawTakeoverBubble(value);const text=raw.replace(/[，,]+$/,'').trim();
    ...
    return text.length<minLength || /[，,、；;：:]$/.test(raw) || /(?:因为|但是|可是|所以|如果|而且|然后|就是|这个|那个|以及|或者|还是|并且|不过|至于|关于|让|把|被|从|向|对|跟|和|比|更|还|却|才|就|也|都|很|太)$/.test(text) || unfinishedPhrase.test(text) || (/[（(《【]/.test(text)&&!/[）)》】]/.test(text));
  }
  function splitCompleteTakeoverMessages(rows) {
    ...
    if(parts.length<3){
      const clauses=parts.flatMap(text=>text.split(/(?<=[。！？!?；;])\s*/).map(item=>item.trim()).filter(Boolean));
      if(clauses.length>=3)return clauses.length<=5?clauses:[clauses[0],clauses[1],clauses.slice(2,-2).join(''),clauses.at(-2),clauses.at(-1)].filter(Boolean);
    }
  ...
  reverseWait = async function(ms) {
    if (ms === 2800 && reverseBusy && reverseLive?.app === '聊天' && reverseLive.stage === 'viewed') {
  ...
  const reverseCheatObserver = new MutationObserver(() => {
    ...
    phone.querySelectorAll('.ta-reverse-desktop-pages main > article').forEach((item,index)=>{const key=choices[index]?.[0];if(!key)return;item.dataset.taReversePrefer=key;item.classList.toggle('is-selected',reversePreferredApp===key);});
    const footer=phone.querySelector('footer span');const preferredLabel=reverseUserApps().find(item=>item[0]===reversePreferredApp)?.[1]||reversePreferredApp;const footerText=reversePreferredApp?`已暗选“${preferredLabel}”，TA 将打开它…`:reversePreferenceUsed?'已按你的暗示查看；接下来由 TA 自行选择…':'点击一个 App 暗中指定；不点击则由 TA 随机选择…';if(footer&&footer.textContent!==footerText)footer.textContent=footerText;
  });
  ...
  document.addEventListener('click', event => {
    const button=event.target.closest?.('[data-ta-reverse-prefer]')||event.target.closest?.('.ta-reverse-desktop-pages main > article');
    if(!button||!app.classList.contains('is-open')||!reverseBusy||reverseLive?.stage!=='thinking')return;
    event.preventDefault();event.stopImmediatePropagation();const fallbackIndex=[...button.parentElement.children].indexOf(button);const selected=button.dataset.taReversePrefer||reverseUserApps()[fallbackIndex]?.[0]||'';if(!selected)return;button.dataset.taReversePrefer=selected;reversePreferredApp=reversePreferredApp===selected?'':selected;
    ...
    const hint=app.querySelector('.ta-reverse-user-phone footer span');const preferredLabel=reverseUserApps().find(item=>item[0]===reversePreferredApp)?.[1]||reversePreferredApp;if(hint)hint.textContent=reversePreferredApp?`已暗选“${preferredLabel}”，TA 将打开它…`:reversePreferenceUsed?'已按你的暗示查看；接下来由 TA 自行选择…':'点击一个 App 暗中指定；不点击则由 TA 随机选择…';
  },true);
  function reverseRoleSpeechGuide(owner,samples){
    const details=String(owner?.details||owner?.signature||owner?.identity||'暂无').replace(/\r/g,'');
    const lines=details.split(/\n+/).map(item=>item.trim()).filter(Boolean);
    const tagged=lines.filter(item=>/(?:说话|语气|口头禅|常用|称呼|用词|词汇|句式|标点|风格|口吻)/.test(item)).slice(0,18);
    const quoted=[...details.matchAll(/[“「『"']([^”」』"']{1,18})[”」』"']/g)].map(item=>item[1].trim()).filter(Boolean).slice(0,20);
    const sampleText=(samples||[]).map(item=>String(item||'').trim()).filter(Boolean).slice(-10).join('\n');
    return [(tagged.length?'角色设定中明确写出的说话规则：\n'+tagged.join('\n'):'角色设定没有单独列出说话规则，请从完整人设谨慎推断，不要擅自添加口头禅。'),quoted.length?'角色设定中的专属词/称呼候选：'+quoted.join('、'):'角色设定没有明确的专属词/称呼。',sampleText?'角色近期真实台词样本（优先模仿节奏，不得照抄）：\n'+sampleText:'暂无真实台词样本'].join('\n');
  }
})();
