(() => {
  ...
  function captureTaChatListScroll() {
    if (activeApp !== 'liaotian' || activeChatTarget) return;
...
可用 NPC：${existing.map(item => `${item.name}（${item.identity || item.reason || '关系未知'}）`).join('；') || '根据角色设定选择确实有关的人物'}`;
  ...
  async function refreshSelectedApp(owner, key, options = {}) {
    ...
    const existingMusic = key === 'music' ? (Array.isArray(snapshot(owner).music) ? snapshot(owner).music : []) : [];
    const musicContext = existingMusic.length ? existingMusic.slice(-20).map(item => `${item.title || item.name || ''}｜${item.artist || item.singer || ''}`).join('、') : '暂无已有歌曲';
    ...
    try {
      ...
      if (key === 'music') savedRows = await hydrateRoleMusic(mergeRoleMusic(previous.music, savedRows));
      if (key === 'calendar') {
  ...
  function roleMusic(owner) {
    ...
    return `<section class="ta-recent-playlist"><header><div><small>RECENTLY PLAYED</small><h2>${esc(owner.nickname || owner.name)} 最近在听</h2></div><span>${rows.length} 首</span></header><div>${rows.map((item, index) => { const count = String(item.playCount || item.plays || item.count || '1次'); return `<button type="button" data-ta-detail="music" data-ta-detail-index="${index}"><em>${index + 1}</em><i>${item.cover ? `<img src="${esc(item.cover)}" alt="">` : builtinAppIcon('yinyue', icon('music'))}</i><span><b>${esc(item.title || item.name || '未知歌曲')}</b><small>${esc(item.artist || '未知歌手')}</small></span><strong>听了 ${esc(/次$/.test(count) ? count : `${count}次`)}</strong><u>›</u></button>`; }).join('')}</div></section>`;
  }
