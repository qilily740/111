(() => {
  ...
  function chatMessageContentForApi(message) {
    if (!message) return '';
    if (message.recalled) return `[${chatParticipantName(message.role)}撤回了一条消息]`;
    if (message.type === 'image') return message.sticker ? `[表情包${message.stickerDescription ? `：${message.stickerDescription}` : ''}]` : '[图片]';
    if (message.type === 'image-desc') return `[图片描述] ${message.text || ''}`.trim();
    if (message.type === 'voice') return `[语音消息] ${message.voiceText || message.text || ''}`.trim();
    if (message.type === 'location') return `[位置分享] ${message.locationName || message.text || ''}${message.locationDetail ? `（${message.locationDetail}）` : ''}`.trim();
    if (message.type === 'transfer') return `[转账] 金额：${message.amount || message.text || ''}，备注：${message.note || '无'}，状态：${message.status || '待处理'}`;
    if (message.type === 'video') return `[视频通话] ${message.text || ''}`.trim();
    if (message.type === 'together') return `[一起听] ${message.text || ''}`.trim();
  ...
  function normalizeCardGender(value) {
    ...
    if (/^(男|男性|male|man|boy|m)$/.test(lower)) return '男';
    if (/^(女|女性|female|woman|girl|f)$/.test(lower)) return '女';
    if (/^(其他|未知|非二元|non[- ]?binary|nonbinary|other|非二元性别)$/.test(lower)) return '其他';
    return text;
  }
...
世界书中的内容是角色所处世界的背景规则。只有在相关时使用，不要主动提到“世界书”，也不要为了展示设定而强行引用。当前聊天记录由系统作为后续消息提供，必须结合最近对话回应，而不是只重复角色资料。
...
7. 默认只回复一条自然、口语化的短消息。日常寒暄、玩笑、追问、撒娇、吃醋或简单回应时，几个字就可以，例如“嗯？”“干嘛”“你认真的？”“行，等我”。不要为了显得丰富而强行展开。
8. 只有当内容中确实有两个或以上自然分开的想法、情绪或动作时，才使用连续消息。连续消息的条数和格式以本次系统追加的要求为准；没有追加要求时不要主动拆分。
9. 每条连续消息都应像单独发出的聊天气泡，而且长短要自然错落：多数可以只有 2—18 个汉字，偶尔一条 19—45 个汉字；确实需要解释、安慰或推进情节时才可以更长。不要让每条长度相近，也不要把一段长文平均切开。短反应必须有语境意义，不要用无意义的“嗯”“……”凑数。
...
14. 回复长度跟随当下语境，而不是每轮固定。用户只说了很短的一句、普通闲聊或情绪反应时，优先回 2—18 个汉字；只有用户认真倾诉、询问复杂问题、发生争执或剧情确实需要时，才展开到较长回复。不要复述用户的话后再解释一遍，也不要每次都交代完整心理过程。
...
用户自定义提示词已经作为“补充规则”提供。请在不破坏以上身份、人设、记忆、世界书、用户自主权和动作格式的前提下使用它；如果自定义内容没有明确要求，不要擅自改变默认的一条短消息回复方式。`;
  ...
  function openContactEditor(contact) { if (contactSaving) return; editorMode = contact ? 'edit' : 'add'; editorContactId = contact?.id || null; editorAvatar = contact?.avatar || ''; editorDraft = null; editorWorldbookDraft = null; renderEditor(); document.querySelector('#contactName')?.focus(); }
...
现在只输出心声正文。`;
  ...
  function offlineMeetingMessages(session, contact) {
    ...
    const items = session.messages.filter(item => !item.contextPrompt).map(item => {
      ...
      const speaker = isUser ? '<div class="chat-offline-v2-user-avatar">你</div>' : avatarMarkup(contact, 'chat-offline-v2-avatar');
      return `<article class="${isUser ? 'is-user' : 'is-character'}">${speaker}<div class="chat-offline-v2-message-body"><span>${isUser ? '你' : esc(roleName)}</span><p>${esc(item.text || '')}</p></div></article>`;
    }).join('');
  ...
  function addMessage(text, role = 'user', type = '', meta = {}) { const chat = currentChat(); if (!chat) return; const raw = String(text || ''); const profileId = chat.profileId || ''; const transfer = role === 'character' ? raw.match(/\[\[TRANSFER\s+amount\s*=\s*([\d.,]+)\s+note\s*=\s*([^\]]*)\]\]/i) : null; if (transfer) { const remaining = raw.replace(transfer[0], '').trim(); const transferNote = transfer[2].trim(); chat.messages.push({ id: uid('message'), text: transferNote, role, type: 'transfer', amount: transfer[1], note: transferNote, status: 'pending', profileId, time: time() }); if (remaining) chat.messages.push({ id: uid('message'), text: remaining, role, type: '', profileId, time: time() }); } else chat.messages.push({ id: uid('message'), text: raw, role, type, profileId, ...meta, time: time() }); save(); render(); setTimeout(() => { const box = document.querySelector('#chatMessages'); if (box) box.scrollTop = box.scrollHeight; }, 0); }
  ...
  function formatLocationDistances(root = document) { root.querySelectorAll?.('.chat-location-distance').forEach(item => { if (item.dataset.locationDistanceFormatted === 'true') return; item.dataset.locationDistanceFormatted = 'true'; const raw = item.textContent.trim(); const match = raw.match(/^([+-]?(?:\d+(?:\.\d+)?|\.\d+))\s*(.*)$/); if (!match) { item.classList.add('is-text'); item.textContent = raw; return; } item.textContent = ''; const value = document.createElement('b'); value.textContent = match[1]; item.appendChild(value); if (match[2]) { const label = document.createElement('small'); label.textContent = match[2]; item.appendChild(label); } }); }
  ...
  function sendCharacterReplyContent(text, chat) {
    ...
    let sent = 0;
    chunks.slice(0, target).forEach(chunk => {
      let cursor = 0;
      const marker = /\[\[STICKER\s*:\s*([^\]]+)\]\]/ig;
      let match;
      while ((match = marker.exec(chunk))) {
        const before = chunk.slice(cursor, match.index).trim();
        if (before && sent < target) { baseCharacterAddMessage(before, 'character'); sent += 1; }
        const item = byId.get(match[1].trim());
        if (item && sent < target) { baseCharacterAddMessage(item.url, 'character', 'image', { sticker: true, stickerDescription: item.text || '' }); sent += 1; }
        cursor = marker.lastIndex;
      }
      const rest = chunk.slice(cursor).trim();
      if (rest && sent < target) { baseCharacterAddMessage(rest, 'character'); sent += 1; }
    });
    if (!sent) { baseCharacterAddMessage(String(text || '……').replace(/\[\[STICKER\s*:[^\]]+\]\]/ig, '').trim() || '……', 'character'); }
  }
...
短小不代表敷衍：可以只回“嗯？”“干嘛”“你认真的？”“行，等我”这类有语境的短句。保持角色的性格、关系、口癖和情绪连续。只有解释复杂事情、认真安慰、争执或推进重要情节时才展开；不要把一段小作文平均切成几段，也不要为了满足数量破坏自然对话。
...
不要把控制标记展示给用户。`;
  ...
  reply = async function() {
    ...
    const memory = window.IdealMachineMemory;
    const chat = currentChat();
    const memoryRoleId = activeContact;
      ...
      try {
        if (String(input).includes('/chat/completions')) {
          ...
          if (system && (String(system.content || '').includes('理想机角色扮演协议') || String(system.content || '').includes('你正在理想机中扮演'))) {
            ...
            const shortMessages = context.shortMessages?.length ? context.shortMessages : payload.messages.filter(item => item !== system);
