(() => {
  ...
  function assistantActions(message, disabled = false) {
    ...
    return `<footer class="doubao-answer-actions"><button type="button" aria-label="复制">${icon('copy')}</button><button type="button" aria-label="朗读">${icon('voice')}</button><button type="button" aria-label="喜欢">${icon('like')}</button><button type="button" aria-label="不喜欢">${icon('dislike')}</button><button type="button" aria-label="分享">${icon('share')}</button><button class="doubao-reroll-button" type="button" aria-label="重roll" ${reroll} ${disabled ? 'disabled' : ''}>${icon('retry')}</button></footer>`;
  }
