(() => {
  ...
  const historyKey = 'ideal-machine-doubao-history';
  ...
  window.IdealMachineDoubaoStyle = DOUBAO_STYLE;
  ...
  function readHistory() {
    try { const value = JSON.parse(localStorage.getItem(historyKey) || '[]'); return Array.isArray(value) ? value : []; }
    catch { return []; }
  }

  function saveHistory(rows) { localStorage.setItem(historyKey, JSON.stringify(rows.slice(0, 100))); }

  function historyTitle(rows) {
    const first = rows.find(item => item.role === 'user')?.content || '豆包对话';
    const clean = String(first).replace(/\s+/g, ' ').trim();
    return clean.length > 22 ? `${clean.slice(0, 22)}…` : clean;
  }

  function saveCurrentConversation() {
    if (sending) return window.alert('豆包还在回复，请等回复完成后再保存。');
    const rows = messages.filter(item => item.role === 'user' || item.role === 'assistant');
    if (!rows.length) return window.alert('当前还没有可以保存的对话。');
    const history = readHistory();
    history.unshift({ id:uid('doubao-history'), savedAt:Date.now(), title:historyTitle(rows), messages:rows.map(item => ({ ...item })) });
    saveHistory(history);
    messages = [];
    viewingHistoryId = '';
    historyOpen = false;
  ...
  function buildRounds(source = messages) {
    const rounds = [];
    for (let index = 0; index < source.length;) {
      const first = source[index];
      const roundMessages = [first];
      if (first.role === 'user' && source[index + 1]?.role === 'assistant') roundMessages.push(source[index + 1]);
  ...
  function selectionHeader() {
    if (!selectionMode) return `<header class="doubao-header"><button class="doubao-menu-button" data-doubao-menu type="button" aria-label="历史记录" title="历史记录">${icon('menu')}</button><div class="doubao-chat-title"><p>AI 生成可能有误 注意核实</p></div><div class="doubao-header-tools"><button type="button" data-doubao-save aria-label="保存当前对话" title="保存对话" ${sending || viewingHistoryId ? 'disabled' : ''}>${icon('save')}</button><button type="button" data-doubao-close aria-label="关闭豆包" title="关闭豆包">×</button></div></header>`;
  ...
  async function shareHistoryToContact(contactId) {
    const record = readHistory().find(item => item.id === shareHistoryId);
    if (!record) return window.alert('没有找到这条历史对话。');
    if (!window.IdealMachineChatShare?.shareDoubaoConversation) return window.alert('聊天 App 尚未准备好，请刷新页面后重试。');
    try {
      const result = await window.IdealMachineChatShare.shareDoubaoConversation({ contactId, title:record.title, messages:record.messages || [], savedAt:record.savedAt });
      if (result?.ok === false) throw new Error(result.message || '分享失败');
      shareHistoryId = '';
      historyActionId = '';
      historyOpen = false;
      render();
  ...
  function render() {
    ...
    const viewingHistory = viewingHistoryId ? readHistory().find(item => item.id === viewingHistoryId) : null;
    const displayMessages = viewingHistory ? (viewingHistory.messages || []) : messages;
    displayMessages.forEach(message => { if (!message.id) message.id = uid(); });
    const renderedMessages = buildRounds(displayMessages).map(round => round.messages.map((message, messageIndex) => {
      const messageId = message.id;
      const selected = !viewingHistory && selectionMode && selectedRoundIds.has(round.id);
      const selectedClass = selected ? ' is-round-selected' : '';
      const check = !viewingHistory && selectionMode && messageIndex === 0 ? `<span class="doubao-round-check">${selected ? '✓' : ''}</span>` : '';
      const selectable = viewingHistory ? '' : ` data-doubao-message-id="${esc(messageId)}" data-doubao-round-id="${esc(round.id)}" data-doubao-round-select`;
      if (message.role === 'recalled') return `<article class="doubao-message is-recalled${selectedClass}"${selectable}>${check}<p>${formatText(message.content)}</p></article>`;
      const quote = messageQuote(message);
      const answerActions = viewingHistory || selectionMode || message.role === 'user' ? '' : assistantActions(message, rerollingMessageId === message.id);
      const content = rerollingMessageId === message.id ? '正在重roll…' : message.content;
      return `<article class="doubao-message ${message.role === 'user' ? 'is-user' : 'is-assistant'}${selectedClass}"${selectable}>${check}${quote}<p>${formatText(content)}</p>${answerActions}</article>`;
    }).join('')).join('');
  ...
  document.addEventListener('click', event => {
    ...
    if (event.target.closest('[data-doubao-menu]')) { historyOpen = !historyOpen; historyActionId = ''; shareHistoryId = ''; render(); return; }
    if (event.target.closest('[data-doubao-close]')) { historyOpen=false; historyActionId=''; shareHistoryId=''; editingMessageId=''; app.classList.remove('is-open'); return; }
    ...
    if (quick) {
      ...
      if (input) input.value=quick.dataset.doubaoQuick;
    }
  });
  ...
  document.addEventListener('keydown', event => {
    if (event.key === 'Escape' && app.classList.contains('is-open') && (shareHistoryId || historyOpen || viewingHistoryId)) {
      if (shareHistoryId) shareHistoryId = ''; else if (historyOpen) historyOpen = false; else viewingHistoryId = '';
      historyActionId = '';
      render();
      return;
    }
