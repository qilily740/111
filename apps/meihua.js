(() => {
  ...
  function applyFullscreen(enabled = saved.fullscreen === true) {
    ...
    if (!isEnabled) {
      ...
      document.documentElement.style.removeProperty('--fullscreen-top-image');
      if (themeColor) themeColor.content = '#f4f4f2';
    } else if (themeColor) {
      themeColor.content = readableTopColor(getComputedStyle(document.body).backgroundColor) || '#f4f4f2';
    }
  ...
  function updateFullscreenTopColor() {
    ...
    const useWallpaper = !color && wallpaper && wallpaper !== 'none';
    document.documentElement.style.setProperty('--fullscreen-top-color', color || 'transparent');
    document.documentElement.style.setProperty('--fullscreen-top-image', useWallpaper ? wallpaper : 'none');
    const themeColor = document.querySelector('#appThemeColor');
    const bodyColor = readableTopColor(getComputedStyle(document.body).backgroundColor);
    if (themeColor) themeColor.content = color || bodyColor || '#f4f4f2';
  }
