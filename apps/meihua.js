(() => {
  ...
  const normalizeLauncherIcon = value => ({ preset: launcherPresets[value?.preset] ? value.preset : 'orbit', appearance: value?.appearance === 'night' ? 'night' : 'day', custom: typeof value?.custom === 'string' ? value.custom : '' });
  ...
  modal.innerHTML = `
    ...
    <section class="beauty-sheet" role="dialog" aria-modal="true" aria-labelledby="beautyTitle">
      ...
      <div class="beauty-content">
        ...
        <section class="beauty-section">
          <div class="beauty-section-head"><div><h3 class="beauty-section-title">主屏幕理想机图标</h3><span class="beauty-launcher-hint">用于浏览器“添加到主屏幕”时的理想机图标</span></div></div>
          <div class="beauty-launcher-panel" id="beautyLauncherIcon"></div>
        </section>
        <section class="beauty-section">
  ...
  function applyLauncherIcon(choice = normalizeLauncherIcon(saved.launcherIcon)) {
    const setIcon = source => {
      if (!source) return;
      document.querySelector('#idealMachineFavicon')?.setAttribute('href', source);
      document.querySelector('#idealMachineAppleTouchIcon')?.setAttribute('href', source);
    };
  ...
  function renderLauncherIconControls() {
    const panel = modal.querySelector('#beautyLauncherIcon');
    if (!panel) return;
    const choice = launcherIconDraft;
    panel.innerHTML = `<div class="beauty-launcher-grid">${Object.entries(launcherPresets).map(([id, preset]) => `<button class="beauty-launcher-choice ${!choice.custom && choice.preset === id ? 'is-selected' : ''}" type="button" data-beauty-launcher-preset="${id}" aria-label="使用第 ${preset.name} 套图标"><img src="${preset[choice.appearance]}" alt="第 ${preset.name} 套图标"><b>${preset.name}</b></button>`).join('')}<label class="beauty-launcher-upload ${choice.custom ? 'is-selected' : ''}"><span class="beauty-launcher-custom-preview">${choice.custom && !choice.custom.startsWith('idb:image:') ? `<img src="${esc(choice.custom)}" alt="自定义图标">` : '＋'}</span><b>上传</b><input class="beauty-file" data-beauty-launcher-file type="file" accept="image/png,image/jpeg,image/webp"></label></div><label class="beauty-launcher-mode"><span><b>图标版本</b><small>添加到主屏幕前，选择要使用的日间或夜间版本。</small></span><select data-beauty-launcher-appearance><option value="day" ${choice.appearance === 'day' ? 'selected' : ''}>日间</option><option value="night" ${choice.appearance === 'night' ? 'selected' : ''}>夜间</option></select></label><p class="beauty-launcher-note">保存后，请删除旧的理想机主屏幕图标，再从浏览器重新“添加到主屏幕”，新图标才会生效。</p>`;
  ...
  async function save() {
    ...
    if (String(launcherIconDraft.custom || '').startsWith('data:image/') && window.IdealMachinePutImage) launcherIconDraft.custom = await window.IdealMachinePutImage(launcherIconDraft.custom);
    saved.launcherIcon = normalizeLauncherIcon(launcherIconDraft);
  ...
  function close() { closeIconPicker(); launcherIconDraft = normalizeLauncherIcon(saved.launcherIcon); applySettings(); modal.classList.remove('is-open'); }
  ...
  modal.addEventListener('click', event => {
    const preset = event.target.closest('[data-beauty-launcher-preset]');
    if (!preset) return;
    launcherIconDraft.preset = preset.dataset.beautyLauncherPreset;
    launcherIconDraft.custom = '';
    renderLauncherIconControls();
    applyLauncherIcon(launcherIconDraft);
  });
  modal.addEventListener('change', async event => {
    const appearance = event.target.closest('[data-beauty-launcher-appearance]');
    if (appearance) {
    ...
    const file = event.target.closest('[data-beauty-launcher-file]');
