(() => {
  ...
  function openApp(fromCreativeFolder = false) { openedFromCreativeFolder = Boolean(fromCreativeFolder); if (!fullTurnBusy) { state = readState(); let recovered = false; state.debates.forEach(item => { if (item.aiGenerating) { item.aiGenerating = false; item.generationError = item.generationError || '上次生成未完成，可以重新尝试。'; recovered = true; } }); if (recovered) save(); } if (page === 'room' && !activeDebate()) { page = 'home'; activeId = ''; } shell?.classList.remove('is-open'); shell?.setAttribute('aria-hidden', 'true'); folder?.classList.remove('is-open'); folder?.setAttribute('aria-hidden', 'true'); app.classList.add('is-open'); app.setAttribute('aria-hidden', 'false'); render(true); }
  ...
  async function generateTurn() {
    ...
    try {
      const response = await fetch(`${config.endpoint.replace(/\/$/, '')}/chat/completions`, { idealScope:'debate', timeout:180000, method:'POST', headers:{ 'Content-Type':'application/json', Authorization:`Bearer ${config.key}` }, body:JSON.stringify({ model, temperature:.82, messages:[
  ...
  async function generateFullDebate() {
    ...
    try {
      ...
      try {
        const judgeResponse = await fetch(`${config.endpoint.replace(/\/$/, '')}/chat/completions`, { idealScope:'debate', timeout:180000, method:'POST', headers:{ 'Content-Type':'application/json', Authorization:`Bearer ${config.key}` }, body:JSON.stringify({ model, temperature:.2, messages:[
      ...
      fullTurnBusy = false;
      render();
    } catch (error) { fullTurnBusy = false; debate.aiGenerating = false; debate.generationError = error?.name === 'AbortError' ? '请求被中断。' : String(error?.message || '请检查 API 配置后重试。'); save(); window.alert(`完整辩论生成失败：${debate.generationError}`); render(); }
  }
