(() => {
  ...
  const items = [
    ...
    { id: '3d-sofa-rose', name: '玫瑰粉法式沙发', cat: '客厅', price: 225, art: 'sofa', color: 'pink', tip: '带滚边靠背的3D客厅沙发', is3d: true, slot: 'living_sofa', area: 'living', variant: 'rose' },
    { id: '3d-sofa-sage', name: '鼠尾草绒面沙发', cat: '客厅', price: 235, art: 'sofa', color: 'green', tip: '低饱和鼠尾草绿色客厅沙发', is3d: true, slot: 'living_sofa', area: 'living', variant: 'sage' },
    { id: '3d-bed-cream', name: '奶油软包大床', cat: '卧室', price: 295, art: 'bed', color: 'cream', tip: '奶油色软包床头与床品', is3d: true, slot: 'bedroom_bed', area: 'bedroom', variant: 'cream' },
    { id: '3d-wardrobe-rose', name: '玫瑰粉雕花衣柜', cat: '卧室', price: 250, art: 'wardrobe', color: 'pink', tip: '带简洁面板装饰的双门衣柜', is3d: true, slot: 'bedroom_wardrobe', area: 'bedroom', variant: 'rose' },
    { id: '3d-shelf-rose', name: '玫瑰木拱顶书架', cat: '书房', price: 225, art: 'shelf', color: 'pink', tip: '拱顶造型与厚实层板', is3d: true, slot: 'study_bookshelf', area: 'study', variant: 'rose' },
    { id: '3d-tv-rose', name: '玫瑰木画框电视', cat: '客厅', price: 245, art: 'tv', color: 'pink', tip: '适合暖色客厅的画框艺术电视', is3d: true, slot: 'living_tv', area: 'living', variant: 'rose' },
    { id: '3d-fridge-rose', name: '灰粉复古冰箱', cat: '厨房', price: 285, art: 'fridge', color: 'pink', tip: '复古圆角柜体与金属把手', is3d: true, slot: 'kitchen_fridge', area: 'dining', variant: 'rose' },
    { id: '3d-vanity-lilac', name: '丁香紫梳妆台', cat: '卧室', price: 255, art: 'desk', color: 'purple', tip: '椭圆镜面与简洁抽屉', is3d: true, slot: 'bedroom_vanity', area: 'bedroom', variant: 'lilac' },
    { id: '3d-lounge-cream', name: '奶油弧形会客沙发', cat: '会客厅', price: 335, art: 'sofa', color: 'cream', tip: '柔和奶油色的会客区组合', is3d: true, slot: 'bay_lounge', area: 'living', variant: 'cream' },
    { id: '3d-bath-lilac', name: '丁香紫浴室套装', cat: '浴室', price: 395, art: 'bath', color: 'purple', tip: '带浴缸、洗手台和马桶的完整套装', is3d: true, slot: 'bathroom_set', area: 'bathroom', variant: 'lilac' },
    { id: '3d-kitchen-island', name: '奶油厨房中岛', cat: '厨房', price: 260, art: 'kitchen', color: 'cream', tip: '带台面、抽屉和两张高脚凳的厨房中岛', is3d: true, slot: '', variant: 'island' },
    { id: '3d-kitchen-cart', name: '灰粉移动餐车', cat: '厨房', price: 185, art: 'kitchen', color: 'pink', tip: '带轮子的收纳餐车与小置物架', is3d: true, slot: '', variant: 'cart' },
    { id: '3d-writing-desk', name: '奶白法式书桌', cat: '书房', price: 210, art: 'desk', color: 'cream', tip: '带抽屉和细腿的书桌', is3d: true, slot: '', variant: 'writing' },
    { id: '3d-rug-lavender', name: '薰衣草花纹地毯', cat: '装饰', price: 125, art: 'rug', color: 'purple', tip: '柔和低饱和的椭圆地毯', is3d: true, slot: '', variant: 'lavender' },
    { id: '3d-wall-painting', name: '法式花园挂画', cat: '装饰', price: 145, art: 'picture', color: 'pink', tip: '带厚实画框的花园风景画', is3d: true, slot: '', variant: 'garden' },
    { id: '3d-floor-lamp', name: '奶油落地灯', cat: '灯具', price: 155, art: 'lamp', color: 'cream', tip: '暖光落地灯与弧形灯罩', is3d: true, slot: '', variant: 'floor' },
  ...
  function createShopPreviewModel(id, THREE) {
    ...
    const palette = {
      cream: material(0xe9e1d7, .9), mauve: material(0x8f818d, .92), sage: material(0x9cae9d, .9), rose: material(0xb98591, .9),
    ...
    const sofaMauve = id.includes('mauve'), sofaSage = id.includes('sage');
    if (id.startsWith('3d-sofa')) {
      const body = sofaSage ? palette.sage : sofaMauve ? palette.mauve : id.includes('rose') ? palette.rose : palette.cream, accent = sofaMauve || sofaSage ? palette.lavender : palette.trim;
      box([3.2, .48, 1.12], [0, .55, 0], body); legSet(3.1, 1.05, .24, palette.darkWood);
      if (sofaMauve || sofaSage) {
      ...
      const wing = id.includes('lavender'), fabric = id.includes('cream') ? palette.cream : wing ? palette.lavender : palette.rose;
      ...
      const panelled = id.includes('mauve'), rosePanel = id.includes('rose'), body = panelled ? palette.mauve : rosePanel ? palette.rose : palette.cream, trim = panelled || rosePanel ? palette.trim : palette.wood;
      ...
      const arched = id.includes('cream'), roseShelf = id.includes('rose'), body = arched ? palette.cream : roseShelf ? palette.rose : palette.wood, accent = arched || roseShelf ? palette.rose : palette.trim;
      ...
      const retro = id.includes('mauve'), body = retro ? palette.mauve : id.includes('rose') ? palette.rose : palette.cream;
      ...
      const folding = id.includes('cream'), body = folding ? palette.cream : id.includes('lilac') ? palette.lavender : palette.rose;
    ...
    } else if (id === '3d-kitchen-island') {
      box([3.2, .82, 1.28], [0, .58, 0], palette.cream); box([3.34, .14, 1.4], [0, 1.06, 0], palette.trim);
      for (const x of [-.92, 0, .92]) box([.62, .34, .05], [x, .7, .67], palette.wood);
      for (const x of [-.92, .92]) { cyl(.28, .62, [x, .31, 1.02], palette.mauve); cyl(.17, .18, [x, .1, 1.02], palette.metal); }
    } else if (id === '3d-kitchen-cart') {
      box([1.5, .12, .72], [0, 1.48, 0], palette.rose); box([1.42, .12, .68], [0, .74, 0], palette.rose); box([1.58, .12, .8], [0, .12, 0], palette.trim);
      for (const x of [-.58, .58]) for (const z of [-.25, .25]) { box([.08, 1.32, .08], [x, .78, z], palette.metal); cyl(.12, .08, [x, .04, z], palette.metal, [Math.PI / 2, 0, 0]); }
    } else if (id === '3d-writing-desk') {
      box([2.7, .16, 1.0], [0, 1.18, 0], palette.cream); box([1.0, .48, .88], [-.72, .88, 0], palette.wood);
      for (const x of [-1.15, 1.15]) for (const z of [-.34, .34]) box([.1, 1.08, .1], [x, .55, z], palette.trim);
      box([.58, .05, .42], [.62, 1.3, .12], palette.glass); box([.38, .05, .28], [-.7, 1.3, .12], palette.rose);
    } else if (id === '3d-rug-lavender') {
      const rug = cyl(1.7, .08, [0, .04, 0], palette.lavender, [0, 0, 0], 48); rug.scale.z = .62;
      const trim = cyl(1.5, .025, [0, .095, 0], palette.rose, [0, 0, 0], 48); trim.scale.z = .62;
    } else if (id === '3d-wall-painting') {
      box([2.6, 1.8, .18], [0, 1.1, 0], palette.wood); box([2.26, 1.46, .06], [0, 1.1, .13], palette.rose);
      for (const x of [-.72, .72]) box([.06, .8, .04], [x, 1.18, .18], palette.trim);
      box([1.55, .06, .04], [0, .68, .18], palette.trim);
    } else if (id === '3d-floor-lamp') {
      cyl(.5, .08, [0, .04, 0], palette.metal); cyl(.07, 2.2, [0, 1.15, 0], palette.metal); cyl(.18, .08, [0, 2.23, 0], palette.metal);
      const shade = new THREE.Mesh(new THREE.ConeGeometry(.52, .62, 24, 1, true), palette.cream); shade.position.set(0, 2.42, 0); shade.rotation.x = Math.PI; group.add(shade);
      const bulb = new THREE.Mesh(new THREE.SphereGeometry(.14, 12, 8), palette.rose); bulb.position.set(0, 2.24, 0); group.add(bulb);
    } else {
  ...
  function finishWork() {
    ...
    ['user', 'role'].forEach(actor => { const task = data.work[actor]; if (!task || Number(task.end) > Date.now()) return; const job = workJobs().find(entry => entry.id === task.jobId); const reward = Math.max(0, Number(task.reward) || (job ? jobReward(job) : 0)); const jobName = job?.name || '专属工作'; data.coins += reward; data.work[actor] = null; data.history.unshift({ id: uid(), text: `${actor === 'role' ? roleName() : userName()} 完成了${jobName}的工作`, coins: reward, time: new Date().toLocaleString('zh-CN', { month: 'numeric', day: 'numeric', hour: '2-digit', minute: '2-digit' }) }); data.history = data.history.slice(0, 10); toast = `+${reward} 家币　${actor === 'role' ? roleName() : userName()} 回家啦`; changed = true; });
    if (changed) save();
  }
  ...
  function room() { return `<section class="house-room-card house-3d-card"><iframe class="house-villa-frame" src="villa-3d/dist/spring-whisper/index.html?v=20260908-scene-perf-7" title="紫色小屋" loading="eager"></iframe></section>`; }
  ...
  function shopPage() { const available = items.filter(entry => entry.is3d === true); const list = cat === '全部' ? available : available.filter(entry => entry.cat === cat); return `${top('家具商城', '点击家具可以360°查看，购买后在仓库应用到3D小家。')}${coins()}<nav class="house-cats">${cats.map(name => `<button class="${cat === name ? 'active' : ''}" data-house-cat="${esc(name)}" type="button">${esc(name)}</button>`).join('')}</nav><section class="house-products">${list.map(info => `<article class="house-product ${info.color} is-3d" data-house-preview-id="${info.id}"><button class="house-product-art" type="button" aria-label="360度查看${esc(info.name)}"><span class="house-3d-badge">3D</span><canvas class="house-product-3d-preview" data-house-model-preview="${info.id}"></canvas><span class="house-product-3d-loading">3D 预览加载中</span><em>360° 查看</em></button><div class="house-product-body"><small>${esc(info.cat)} · 立体家具</small><b>${esc(info.name)}</b><p>${esc(info.tip)}</p><footer><strong>${info.price}<i> 家币</i></strong><button data-house-buy="${info.id}" type="button" ${home().coins < info.price ? 'disabled' : ''}>${home().coins < info.price ? '不足' : '买下'}</button></footer></div></article>`).join('')}</section>`; }
  function storagePage() { const data = home(); const placedTotal = Object.values(data.areas).reduce((sum, area) => sum + area.placed.length, 0) + Object.keys(data.equipped3d || {}).length; return `${top('仓库', '商城购买的3D家具会自动替换小家中的对应家具。')}${coins()}<div class="house-storage-summary"><span>MY 3D FURNITURE</span><b>${data.owned.length} 件　·　已用 ${placedTotal} 件</b></div><section class="house-storage">${data.owned.length ? data.owned.slice().reverse().map(entry => { const info = item(entry.itemId); const isPlaced = placedAnywhere(entry.id); const refund = Math.floor((info?.price || 0) * .6); const actionText = isPlaced ? '收回' : '应用到3D小家'; const actionMarkup = info.slot ? `<button data-house-${isPlaced ? 'remove' : 'place'}="${entry.id}" type="button">${actionText}</button>` : '<span class="house-storage-unavailable">3D 收藏</span>'; return `<article class="house-storage-item is-3d"><div><span class="house-3d-badge">3D</span><canvas class="house-product-3d-preview" data-house-model-preview="${info.id}"></canvas><span class="house-product-3d-loading">3D 预览加载中</span></div><span><b>${esc(info.name)}</b><small>${esc(info.cat)} · ${isPlaced ? `已应用到${areaFor(entry.id)}` : info.slot ? `可替换${slotNames[info.slot] || '小家家具'}` : '可在商城重新查看3D效果'}</small></span><div class="house-storage-actions">${actionMarkup}<button class="recycle" data-house-recycle="${entry.id}" type="button">回收 +${refund}</button></div></article>`; }).join('') : '<div class="house-empty-card"><i>⌂</i><h2>仓库还是空的</h2><p>去商城挑一件喜欢的3D家具吧。</p><button data-house-page="shop" type="button">去商城</button></div>'}</section>`; }
  ...
  function blockVillaPageZoom() { const frame = app.querySelector('.house-villa-frame'); if (!frame) return; const stop = event => { if (event.touches?.length > 1 || event.type.startsWith('gesture') || event.ctrlKey) event.preventDefault(); }; const bind = () => { const doc = frame.contentDocument; if (!doc || doc.documentElement.dataset.zoomGuard) return; doc.documentElement.dataset.zoomGuard = 'true'; ['touchstart','touchmove','wheel','gesturestart','gesturechange','gestureend'].forEach(type => doc.addEventListener(type, stop, { passive:false, capture:true })); }; frame.addEventListener('load', bind, { once:true }); bind(); }
  function render(preserveScroll = false) { const previousPage = preserveScroll ? app.querySelector('.house-page') : null; const scrollTop = previousPage?.scrollTop || 0; house3d?.destroy?.(); shop3dViewer?.destroy?.(); preview3dId = ''; finishWork(); app.innerHTML = `<main class="house-page">${page === 'room' ? roomPage() : page === 'work' ? workPage() : page === 'shop' ? shopPage() : storagePage()}${toast ? `<div class="house-toast">${esc(toast)}</div>` : ''}${nav()}${placementSheet()}${dyeSheet()}</main>`; if (preserveScroll) { const nextPage = app.querySelector('.house-page'); if (nextPage) nextPage.scrollTop = scrollTop; } if (page === 'room') { const tabs = app.querySelector('.house-area-tabs'); const activeTab = tabs?.querySelector('.active'); if (tabs && activeTab) tabs.scrollLeft = Math.max(0, activeTab.offsetLeft - (tabs.clientWidth - activeTab.offsetWidth) / 2); blockVillaPageZoom(); initThreeHome(); } else if (page === 'shop' || page === 'storage') requestAnimationFrame(initShop3dPreviews); if (toast) { clearTimeout(toastTimer); toastTimer = setTimeout(() => { toast = ''; render(true); }, 2400); } }
  ...
  function start(actor, jobId) { if (actor === 'role' && !character()) return; const data = home(); if (data.work[actor]) return; const job = workJobs(actor).find(entry => entry.id === jobId); if (!job) return; const duration = jobTime(job); const reward = jobReward(job); data.work[actor] = { jobId, duration, reward, end: Date.now() + duration * 1000 }; save(); flash(`${actor === 'role' ? roleName() : userName()} 开始工作了`, true); }
  ...
  document.addEventListener('touchmove', event => { if (app.classList.contains('is-open') && page === 'room' && event.touches.length > 1) event.preventDefault(); }, { passive:false, capture:true });
  document.addEventListener('wheel', event => { if (app.classList.contains('is-open') && page === 'room' && event.ctrlKey) event.preventDefault(); }, { passive:false, capture:true });
  ['gesturestart','gesturechange','gestureend'].forEach(type => document.addEventListener(type, event => { if (app.classList.contains('is-open') && page === 'room') event.preventDefault(); }, { passive:false, capture:true }));
