(() => {
  ...
  function room() { return `<section class="house-room-card house-3d-card"><iframe class="house-villa-frame" src="villa-3d/dist/spring-whisper/index.html?v=20260906-blender-fountain-1" title="紫色小屋" loading="eager"></iframe></section>`; }
  ...
  function render(preserveScroll = false) { const previousPage = preserveScroll ? app.querySelector('.house-page') : null; const scrollTop = previousPage?.scrollTop || 0; house3d?.destroy?.(); shop3dViewer?.destroy?.(); preview3dId = ''; finishWork(); app.innerHTML = `<main class="house-page">${page === 'room' ? roomPage() : page === 'work' ? workPage() : page === 'shop' ? shopPage() : storagePage()}${toast ? `<div class="house-toast">${esc(toast)}</div>` : ''}${nav()}${placementSheet()}${dyeSheet()}</main>`; if (preserveScroll) { const nextPage = app.querySelector('.house-page'); if (nextPage) nextPage.scrollTop = scrollTop; } if (page === 'room') { const tabs = app.querySelector('.house-area-tabs'); const activeTab = tabs?.querySelector('.active'); if (tabs && activeTab) tabs.scrollLeft = Math.max(0, activeTab.offsetLeft - (tabs.clientWidth - activeTab.offsetWidth) / 2); initThreeHome(); } else if (page === 'shop' || page === 'storage') requestAnimationFrame(initShop3dPreviews); if (toast) { clearTimeout(toastTimer); toastTimer = setTimeout(() => { toast = ''; render(true); }, 2400); } }
