(() => {
  ...
  function storagePage() { const data = home(); const placedTotal = Object.values(data.areas).reduce((sum, area) => sum + area.placed.length, 0); return `${top('仓库', '买下的家具都在这里，随时可以换个位置。')}${coins()}<div class="house-storage-summary"><span>MY FURNITURE</span><b>${data.owned.length} 件　·　已摆 ${placedTotal} 件</b></div><section class="house-storage">${data.owned.length ? data.owned.slice().reverse().map(entry => { const info = item(entry.itemId); const isPlaced = placedAnywhere(entry.id); const refund = Math.floor((info?.price || 0) * .6); return `<article class="house-storage-item"><div>${art(info?.art, info?.color)}</div><span><b>${esc(info?.name || '家具')}</b><small>${esc(info?.cat || '')} · ${isPlaced ? `已放在${areaFor(entry.id)}` : '仓库中'}</small></span><div class="house-storage-actions"><button data-house-${isPlaced ? 'remove' : 'place'}="${entry.id}" type="button" ${!isPlaced && data.areas[activeArea].placed.length >= maxFurniturePerArea ? 'disabled' : ''}>${isPlaced ? '收回' : '选择区域摆入'}</button><button class="recycle" data-house-recycle="${entry.id}" type="button">回收 +${refund}</button></div></article>`; }).join('') : '<div class="house-empty-card"><i>⌂</i><h2>仓库还是空的</h2><p>去商城挑一件喜欢的家具吧。</p><button data-house-page="shop" type="button">去商城</button></div>'}</section>`; }
