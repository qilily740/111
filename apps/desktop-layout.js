(() => {
  ...
  function readState() {
    try {
      ...
      return { pages:Array.isArray(value.pages) ? value.pages : [], dock:Array.isArray(value.dock) ? value.dock : [], dockPosition:value.dockPosition && typeof value.dockPosition === 'object' ? value.dockPosition : { x:0, y:0 }, hiddenWidgets:Array.isArray(value.hiddenWidgets) ? value.hiddenWidgets : [], widgetCopies:Array.isArray(value.widgetCopies) ? value.widgetCopies : [], folders:Array.isArray(value.folders) ? value.folders : [], positions:value.positions && typeof value.positions === 'object' ? value.positions : {} };
    } catch { return { pages:[], dock:[], dockPosition:{ x:0, y:0 }, hiddenWidgets:[], widgetCopies:[], folders:[], positions:{} }; }
  }
  ...
  function cleanState() {
    ...
    return { pages:resultPages.slice(0, pages.length), dock:resultDock, dockPosition, hiddenWidgets:[...hidden], widgetCopies:(state.widgetCopies || []).filter(copyInfo => copyInfo?.id && itemMap.has(copyInfo.id)), folders, positions:state.positions && typeof state.positions === 'object' ? state.positions : {} };
  }
  ...
  function moveDockGesture(event) {
    ...
    applyDockPosition({ x:dockDrag.startPosition.x + nextLeft - dockDrag.startRect.left, y:dockDrag.startPosition.y + nextTop - dockDrag.startRect.top });
    event.preventDefault();
  }
