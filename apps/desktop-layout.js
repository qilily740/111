(() => {
  ...
  if (!desktop || pages.length < 2 || !dock) return;
  ...
  function readState() {
    try {
      ...
      return { pages:Array.isArray(value.pages) ? value.pages : [], dock:Array.isArray(value.dock) ? value.dock : [], hiddenWidgets:Array.isArray(value.hiddenWidgets) ? value.hiddenWidgets : [], widgetCopies:Array.isArray(value.widgetCopies) ? value.widgetCopies : [], folders:Array.isArray(value.folders) ? value.folders : [], positions:value.positions && typeof value.positions === 'object' ? value.positions : {} };
    } catch { return { pages:[], dock:[], hiddenWidgets:[], widgetCopies:[], folders:[], positions:{} }; }
  }
  ...
  function syncPageGridRowHeights() {
    grids.forEach(grid => {
      ...
      if (!photos) return;
      const span = itemSpan(photos).rows;
      const photoHeight = photos.getBoundingClientRect().height;
      if (!photoHeight || span < 1) return;
      const rowGap = parseFloat(getComputedStyle(grid).rowGap) || 0;
      const requiredRowHeight = Math.ceil((photoHeight - rowGap * (span - 1)) / span) + 7;
      grid.style.setProperty('--desktop-grid-row', `${Math.max(80, requiredRowHeight)}px`);
  ...
  function cleanState() {
    ...
    return { pages:resultPages.slice(0, pages.length), dock:resultDock, hiddenWidgets:[...hidden], widgetCopies:(state.widgetCopies || []).filter(copyInfo => copyInfo?.id && itemMap.has(copyInfo.id)), folders, positions:state.positions && typeof state.positions === 'object' ? state.positions : {} };
  }
  ...
  function visibleGridRows(grid) {
    ...
    const rowHeight = parseFloat(style.gridAutoRows) || parseFloat(style.getPropertyValue('--desktop-grid-row')) || 80;
    const rowGap = parseFloat(style.rowGap) || 0;
    const paddingTop = parseFloat(style.paddingTop) || 0;
    const paddingBottom = parseFloat(style.paddingBottom) || 0;
    return Math.max(1, Math.floor((desktop.clientHeight - paddingTop - paddingBottom + rowGap) / Math.max(1, rowHeight + rowGap)));
  }
  ...
  function syncPageIndicator() {
    ...
    indicator.replaceChildren(...pages.map((page, index) => { const dot = document.createElement('span'); dot.className = `dot${index === current ? ' active' : ''}`; return dot; }));
  ...
  function setEditing(value) {
    ...
    if (!editing) { closeLibrary(); closeAddMenu(); cleanupDrag(false); cleanupFolderDrag(false); createdFolderLayer?.classList.remove('desktop-folder-edit-mode'); }
  }
  ...
  function gridPositionFromPoint(grid, clientX, clientY, item) {
    ...
    const row = Math.max(1, Math.floor(Math.max(0, clientY - rect.top - paddingTop) / (rowHeight + rowGap)) + 1);
    return { column, row };
  }
  ...
  function beginDrag(item, event) {
    ...
    drag = { item, placeholder, ghost, pointerId:event.pointerId, startX:event.clientX, startY:event.clientY, moved:false, pageTurned:false, offsetX:event.clientX - rect.left, offsetY:event.clientY - rect.top, originalContainer:item.parentElement, originalPosition, drop:null };
  }
  ...
  function moveDrag(event) {
    ...
    if (!drag.pageTurned && Date.now() - edgeTurnAt > 650) { const index = currentPage(); if (event.clientX < 34 && index > 0) { desktop.scrollTo({ left:(index - 1) * desktop.clientWidth, behavior:'smooth' }); drag.pageTurned = true; edgeTurnAt = Date.now(); } else if (event.clientX > innerWidth - 34 && index < pages.length - 1) { desktop.scrollTo({ left:(index + 1) * desktop.clientWidth, behavior:'smooth' }); drag.pageTurned = true; edgeTurnAt = Date.now(); } }
    const under = document.elementFromPoint(event.clientX, event.clientY);
    ...
    if (container.matches('.desktop-layout-grid')) {
      const position = gridPositionFromPoint(container, event.clientX, event.clientY, drag.item);
    ...
    const before = candidates.find(item => { const rect = item.getBoundingClientRect(); return event.clientY < rect.top + rect.height / 2 || (event.clientY < rect.bottom && event.clientX < rect.left + rect.width / 2); });
  ...
  document.addEventListener('pointermove', event => { if (folderDrag) moveFolderDrag(event); else if (drag) moveDrag(event); }, { capture:true, passive:false });
  ...
  document.addEventListener('pointercancel', () => { cleanupFolderDrag(false); cleanupDrag(false); }, true);
  ...
  window.addEventListener('resize', () => {
    ...
    gridResizeFrame = requestAnimationFrame(syncPageGridRowHeights);
  });
