(() => {
  ...
  const saveEvents = events => { localStorage.setItem(eventsKey, JSON.stringify(events)); try { const couple = JSON.parse(localStorage.getItem(coupleKey) || '{}'); if (couple.spaces && typeof couple.spaces === 'object') Object.entries(couple.spaces).forEach(([roleId, space]) => { if (space && typeof space === 'object') space.events = events.filter(item => item.contactId === roleId || item.authorId === roleId || item.roleId === roleId); }); else couple.events = events; localStorage.setItem(coupleKey, JSON.stringify(couple)); } catch {} window.IdealMachineRenderCalendar?.(); };
