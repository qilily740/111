(() => {
  ...
  const today = new Date(); let month = new Date(); let selectedDate = `${today.getFullYear()}-${String(today.getMonth() + 1).padStart(2, '0')}-${String(today.getDate()).padStart(2, '0')}`; let editorOpen = false; let syncingAllRoles = false; let roleDetailId = ''; let refreshPickerOpen = false; let selectedRefreshRoleIds = new Set();
  ...
  function refreshPicker() {
    const roles = readChat().contacts;
    const selectedCount = selectedRefreshRoleIds.size;
    const allSelected = roles.length > 0 && selectedCount === roles.length;
    const rows = roles.length ? roles.map(role => `<button class="calendar-refresh-role ${selectedRefreshRoleIds.has(role.id) ? 'is-selected' : ''}" data-calendar-refresh-role="${esc(role.id)}" type="button"><span class="calendar-refresh-avatar">${roleAvatar(role.id)}</span><span><b>${esc(role.name || role.nickname || '未命名角色')}</b><small>${esc(role.nickname && role.name && role.nickname !== role.name ? role.nickname : '角色日程')}</small></span><em>${selectedRefreshRoleIds.has(role.id) ? '✓' : ''}</em></button>`).join('') : '<p class="calendar-refresh-empty">聊天 App 里还没有角色，请先创建角色。</p>';
    return `<div class="calendar-refresh-picker"><button class="calendar-refresh-backdrop" data-calendar-refresh-close type="button" aria-label="关闭"></button><section><header><div><span>ROLE SCHEDULE</span><h2>选择要刷新的角色</h2><p>每选择 1 个角色会调用 1 次 API。本次将调用 <b>${selectedCount}</b> 次。</p></div><button data-calendar-refresh-close type="button" aria-label="关闭">×</button></header>${roles.length ? `<button class="calendar-refresh-all" data-calendar-refresh-all type="button"><i class="${allSelected ? 'is-checked' : ''}">${allSelected ? '✓' : ''}</i><span>全选角色</span><small>${allSelected ? '已全选' : `已选择 ${selectedCount} 个`}</small></button><main>${rows}</main><footer><button data-calendar-refresh-close type="button">取消</button><button class="is-primary" data-calendar-refresh-submit type="button" ${selectedCount ? '' : 'disabled'}>刷新 ${selectedCount || ''} 个角色（${selectedCount} 次 API）</button></footer>` : rows}</section></div>`;
  }
  ...
  document.addEventListener('click', event => {
    if (event.target.closest('[data-app-key="rili"]')) { app.classList.add('is-open'); refreshPickerOpen = false; selectedRefreshRoleIds.clear(); render(); return; }
    ...
    if (event.target.closest('[data-calendar-refresh-close]')) { refreshPickerOpen = false; selectedRefreshRoleIds.clear(); render(); return; }
    if (event.target.closest('[data-calendar-refresh-all]')) { const roles = readChat().contacts; if (selectedRefreshRoleIds.size === roles.length) selectedRefreshRoleIds.clear(); else roles.forEach(role => selectedRefreshRoleIds.add(role.id)); render(); return; }
    const refreshRole = event.target.closest('[data-calendar-refresh-role]'); if (refreshRole) { const id = refreshRole.dataset.calendarRefreshRole; selectedRefreshRoleIds.has(id) ? selectedRefreshRoleIds.delete(id) : selectedRefreshRoleIds.add(id); render(); return; }
    if (event.target.closest('[data-calendar-refresh-submit]')) { const roleIds = [...selectedRefreshRoleIds]; if (!roleIds.length) return; refreshPickerOpen = false; selectedRefreshRoleIds.clear(); runCalendarRefresh(() => window.IdealMachineSyncSelectedRoleCalendars?.(roleIds, selectedDate), true); return; }
    if (event.target.closest('[data-calendar-sync-all]')) { const roles = readChat().contacts; if (!roles.length) return window.alert('聊天 App 里还没有角色，请先创建角色。'); selectedRefreshRoleIds.clear(); refreshPickerOpen = true; render(); return; }
