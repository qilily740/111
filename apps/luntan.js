(() => {
  ...
  const getProfile = () => { const value = read(profileKey, {}); return { nickname: value.nickname || '论坛用户', avatar: value.avatar || '', allowedRoles: Array.isArray(value.allowedRoles) ? value.allowedRoles : [] }; };
  ...
  async function boundRoleForumContext(role, query = '论坛互动与共同经历') { const chatData = read('ideal-machine-chat', {}); const chat = chatData.chats?.[role?.id] || {}; const profile = (chatData.profiles || []).find(item => item.id === chat.profileId) || {}; let history = (chat.messages || []).slice(-12); let memoryText = ''; try { const context = await window.IdealMachineMemory?.prepareContext?.({ roleId:role.id, profileId:chat.profileId || '', chat, query }); if (context) { memoryText = context.systemPrompt || ''; history = context.shortMessages || history; } } catch {} const recent = history.slice(-12).map(message => `${message.role === 'user' ? profile.nickname || profile.realName || '用户' : role.nickname || role.name || '角色'}：${message.content || message.text || ''}`).join('\n'); return `论坛账号“${getProfile().nickname}”就是这位角色在聊天 App 中绑定的同一位用户；请按各自关系承接，不得把对方当陌生人，也不得串用其他角色的记忆。\n${roleUserContext(role).slice(0,3000)}\n该角色自己的长期记忆：\n${(memoryText || '暂无已整理的长期记忆。').slice(0,2200)}\n该角色最近聊天：\n${(recent || '暂无聊天记录。').slice(-1600)}`; }
  function syncForumMemory(role, post, interactionId, userText, roleText, action) { const chatData = read('ideal-machine-chat', {}); const chat = chatData.chats?.[role?.id] || {}; const profileId = chat.profileId || ''; const memory = window.IdealMachineMemory; if (!role?.id || !memory?.readLibrary || !memory?.saveLibrary) return; const sourceId = `forum:${post.id}:${interactionId}`; const library = memory.readLibrary(); if (library.forgottenSourceIds?.includes(sourceId) || library.entries.some(item => item.sourceId === sourceId)) return; library.entries.unshift({ id:uid('memory'), sourceId, roleId:role.id, profileId, roleName:role.nickname || role.name || '角色', type:'forum', level:'long', title:'论坛里的共同互动', summary:`论坛用户“${getProfile().nickname}”发布/评论：“${String(userText || post.text || '').slice(0,180)}”。${role.nickname || role.name || '角色'}${action === 'like' ? '点赞回应了这条动态' : `在论坛回复：“${String(roleText || '').slice(0,180)}”`}${post.text ? `（原动态：${String(post.text).slice(0,120)}）` : ''}`, emotion:'', keywords:['论坛','共同互动'], importance:'日常', importanceScore:.58, createdAt:Date.now() }); memory.saveLibrary(library); }
  ...
  function renderMePage(feed) {
    ...
    feed.querySelector('.forum-home-card-details')?.remove();
    resolveStoredAvatars();
  }
  ...
  async function refreshForumPosts() {
    ...
    const roleContext = (await Promise.all(roles.map(async role => `角色ID：${role.id}\n网名：${role.nickname || role.name || '角色'}\n真实姓名：${role.name || ''}\n${await boundRoleForumContext(role, `为${profile.nickname}生成论坛动态`)}`))).join('\n\n');
  ...
  async function generatePostInteraction(post) {
    ...
    try {
      ...
      results.slice(0, 3).forEach(result => {
        ...
        const userText = parent && (parent.ownerType === 'user' || (!parent.ownerType && parent.nickname === getProfile().nickname)) ? parent.text : (!parent && (target.ownerType === 'user' || target.handle === 'user') ? target.text : '');
        ...
        if ((action === 'reply' || action === 'both') && text) {
          const replyId = uid('forum-reply');
          target.replies.push({ id: replyId, nickname: actor.displayName, avatar: actor.avatar || '', text, time: now(), replyTo:parent?.id || '', replyToName:parent?.nickname || '', ownerType: actor.actorType === 'npc' ? 'npc' : 'character', ownerId: actor.id });
          if (actor.actorType === 'role' && userText) syncForumMemory(actor, target, replyId, userText, text, 'reply');
  ...
  function openProfile() {
    ...
    sheet.innerHTML = '<div class="forum-sheet-backdrop" data-forum-profile-close></div><section class="forum-profile-card"><header><div><span>ACCOUNT</span><h2>论坛账号</h2></div><button type="button" data-forum-profile-close>×</button></header><label class="forum-avatar-choose" data-forum-avatar-trigger><div class="forum-profile-avatar-preview" data-forum-profile-preview>' + avatar(current.avatar, current.nickname) + '</div><span>点击头像更换</span><input type="file" accept="image/*" data-forum-avatar-file></label><label>论坛网名<input data-forum-nickname maxlength="30" value="' + esc(current.nickname) + '" placeholder="输入论坛网名"></label><section class="forum-role-selector"><h3>允许参与论坛的角色</h3><p>读取角色聊天记录，并选择哪些角色可以参与论坛。</p><div>' + roleOptions + '</div></section><p class="forum-profile-note">论坛用户会沿用每位角色各自绑定的聊天关系和记忆。</p><footer><button class="forum-profile-delete" type="button" data-forum-profile-delete>删除账号</button><button type="button" data-forum-profile-close>取消</button><button class="is-primary" type="button" data-forum-profile-save>保存账号</button></footer></section>';
    sheet.classList.add('is-open'); sheet.setAttribute('aria-hidden', 'false'); resolveStoredAvatars();
  }
  ...
  app.addEventListener('click', event => {
    ...
    if (submit) { const text = app.querySelector('[data-forum-text]')?.value.trim(); if (!text) return; const current = getProfile(); const list = getPosts(); list.unshift({ id: uid('forum-post'), nickname: current.nickname, handle: 'user', avatar: current.avatar, text, time: now(), likes: 0, comments: 0, reposts: 0, owner: current.nickname, ownerType: 'user' }); save(feedKey, list); closeCompose(); render(); return; }
