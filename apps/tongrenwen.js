(() => {
  ...
  function roleOptions() {
    const result = [];
    try { const chat = JSON.parse(localStorage.getItem('ideal-machine-chat') || '{}'); (chat.contacts || []).forEach(item => { if (item.id) result.push({ id: `contact:${item.id}`, name: item.name || item.nickname || '角色', nickname: item.nickname || '', type: 'character' }); }); } catch {}
  ...
  function chapterHtml(chapter, index) { const saving = chapterBusyId === chapter.id; return `<article class="fanfic-chapter"><div class="fanfic-chapter-head"><span>CHAPTER ${String(index + 1).padStart(2, '0')}</span><input data-fanfic-chapter-title="${esc(chapter.id)}" value="${esc(chapter.title || `第 ${index + 1} 章`)}" aria-label="章节标题"><small>${chapter.savedAt ? '已加入书架 · ' : ''}${esc(chapter.updatedAt || chapter.createdAt || '')}</small></div><textarea data-fanfic-chapter-content="${esc(chapter.id)}" placeholder="写下这一章……">${esc(chapter.content || '')}</textarea><div class="fanfic-chapter-actions"><button data-fanfic-save-chapter="${esc(chapter.id)}" type="button">保存并加入书架</button><button data-fanfic-reroll-chapter="${esc(chapter.id)}" type="button" ${saving || busy ? 'disabled' : ''}>${saving ? '重roll中…' : '重roll这一章'}</button></div></article>`; }
  ...
  function saveStory(withOpening = false, generated = {}) { collectDraft(); const settings = JSON.parse(JSON.stringify(draft)); const existing = editingId ? activeStory() : null; const story = existing || normalizeStory({ id: uid(), createdAt: now(), chapters: [] }); story.title = generated.title || settings.title || '未命名故事'; story.settings = settings; story.updatedAt = now(); story.onShelf = true; if (generated.opening) story.chapters.push({ id: uid('chapter'), title: generated.chapterTitle || '第一章', content: generated.opening, createdAt: now(), updatedAt: now(), savedAt: now() }); if (!existing) state.stories.push(story); save(); activeId = story.id; editingId = ''; page = withOpening || existing ? 'story' : 'home'; render(); }
  ...
  async function rerollChapter(chapterId) {
    ...
    const model = window.IdealMachineAPI?.getModel?.('fanfic') || window.IdealMachineAPI?.getModel?.('chat');
    if (!config.endpoint || !config.key || !model) return window.alert('请先在设置中配置同人文 API 模型。');
    chapterBusyId = chapterId; render();
    try {
      ...
      const response = await fetch(`${config.endpoint.replace(/\/$/, '')}/chat/completions`, { method:'POST', headers:{'Content-Type':'application/json', Authorization:`Bearer ${config.key}`}, body:JSON.stringify({ model, temperature:.98, messages:[{role:'system',content:'你是同人文章节重roll助手，严格遵循故事设定，只返回合法 JSON。'},{role:'user',content:prompt}] }) });
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
