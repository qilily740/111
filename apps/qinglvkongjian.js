(() => {
  ...
  const mysteryCases = legacyMysteryCases.map(item => { const config = mysteryCaseConfig[item.id]; return { ...item, ...config, cover:'assets/ui/game-covers/ancient-case-scroll-v1.png', clues:config.clues, playerKnownData:{ intro:config.intro, characters:config.characters, initialFacts:config.initialFacts }, unlockConditions:Object.fromEntries(config.investigations.map(investigation => [investigation.id, investigation.requirements])) }; });
  ...
  function games(role, profile) {
    ...
    const installed = Array.from({ length: 3 }, (_, index) => index === 0 ? '<article class="couple-games-installed-card is-filled"><img class="couple-games-installed-cover" src="assets/ui/game-covers/flower-roulette-cover-photo-v2.png" alt="花朵轮盘游戏封面"><div class="couple-games-installed-copy"><i>双人心理博弈</i><b>花朵轮盘</b><span>谁被鲜花打中，谁就输</span><button data-flower-roulette-open type="button">打开</button></div></article>' : index === 1 ? '<article class="couple-games-installed-card is-filled couple-mystery-cover"><div class="couple-mystery-cover-art"><img src="assets/ui/game-covers/ancient-mystery-cover-v2.png" alt="古籍悬案游戏封面"></div><div class="couple-games-installed-copy"><i>和 TA 一起读案推理</i><b>古籍悬案</b><span>翻开案卷，找出真相</span><button data-mystery-open type="button">打开</button></div></article>' : '<article class="couple-games-installed-card is-filled stock-cover"><img class="couple-games-installed-cover" src="assets/ui/game-covers/stock-sort-cover-v1.png" alt="分类理货游戏封面"><div class="couple-games-installed-copy"><i>轻巧分类益智</i><b>分类理货</b><span>整理货堆，完成今日订单</span><button data-stock-open type="button">打开</button></div></article>').join('');
  ...
  function flowerRouletteGunMarkup() {
    const assetVersion = '20260922-gun-cutout-1';
    ...
    const gunImage = actor === 'role'
      ? (flowerShot ? 'gun-bouquet-front-cutout.png' : 'gun-front-cutout.png')
      : (flowerShot ? 'gun-bouquet-diagonal-cutout.png' : 'gun-diagonal-cutout.png');
  ...
  document.addEventListener('click', event => {
    ...
    if (event.target.closest('[data-couple-close]')) { window.IdealMachineCancelRequests?.('couple'); apiBusy = false; flowerRouletteOpen = false; mysteryOpen = false; stockGameOpen=false;stockSelected=-1;stockShipping=[];if(stockShipTimer)window.clearTimeout(stockShipTimer);stockShipTimer=0;resetFlowerRouletteRound(); app.classList.remove('is-open'); return; }
