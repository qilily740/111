...
import {createInteriorPreview} from './interior.js';
...
function botanicalInstances(arr,geometry,material,index){
  ...
  });inst.count=accepted;inst.castShadow=true;inst.receiveShadow=true;house.add(inst);return inst;
}
const botanicalReady=fetch(new URL('./realistic-botanicals.json',import.meta.url)).then(r=>{if(!r.ok)throw new Error(`Botanical asset HTTP ${r.status}`);return r.json()}).then(asset=>{
  ...
  for(let i=0;i<4;i++){
    ...
    bases.name='Blender flower calyx and stalk';bases.instanceMatrix=blooms.instanceMatrix;bases.castShadow=true;bases.receiveShadow=true;house.add(bases);
  }
...
const interiorPreview=createInteriorPreview(scene,house,renderer,camera);
renderer.domElement.addEventListener('pointermove',()=>interiorPreview.update());
const originalRender=renderer.render.bind(renderer);renderer.render=(...args)=>{interiorPreview.update();originalRender(...args)};
...
document.querySelectorAll('[data-weather]').forEach(button=>button.onclick=()=>{weatherSystem.set(button.dataset.weather);document.querySelectorAll('[data-weather]').forEach(item=>item.classList.toggle('active',item===button));updateControlTone();});
const timeRange=document.querySelector('#time-range'),timeOutput=document.querySelector('#time-output'),localHour=()=>{const now=new Date();return now.getHours()+now.getMinutes()/60+now.getSeconds()/3600};let followActualTime=false,selectedHour=12;timeRange.value=selectedHour;
function showTime(){const value=Number(timeRange.value),hour=Math.floor(value),minute=value===24?0:Math.min(59,Math.round((value%1)*60));selectedHour=value;timeOutput.value=`${String(hour).padStart(2,'0')}:${String(minute).padStart(2,'0')}`;updateControlTone();}timeRange.oninput=()=>{followActualTime=false;showTime();};showTime();
...
function updateTimeOfDay(){const date=new Date();if(followActualTime){selectedHour=localHour();timeRange.value=selectedHour;const h=date.getHours(),m=date.getMinutes();timeOutput.value=`${String(h).padStart(2,'0')}:${String(m).padStart(2,'0')}`;updateControlTone();}else{const hour=selectedHour>=24?24:Math.floor(selectedHour),minute=selectedHour>=24?0:Math.min(59,Math.round((selectedHour%1)*60));date.setHours(hour,minute,0,0);}const state=applyDaylight({scene,skyMaterial,hemi,sun,warm,interiorGlow,lampLights,renderer,ground},date);return weatherSystem.applyLighting(state,{scene,skyMaterial,hemi,sun,renderer});}function resize(){camera.aspect=innerWidth/innerHeight;camera.updateProjectionMatrix();renderer.setSize(innerWidth,innerHeight)}addEventListener('resize',resize);if(innerWidth<600)camera.position.multiplyScalar(1.18);const introDuration=2.8,introStart=performance.now(),introEndPosition=camera.position.clone(),introStartPosition=introEndPosition.clone().multiplyScalar(1.42),introEndTarget=controls.target.clone(),introStartTarget=introEndTarget.clone().add(new T.Vector3(0,1.1,0));camera.position.copy(introStartPosition);controls.target.copy(introStartTarget);controls.enabled=false;const loading=document.querySelector('#loading'),animationClock=new T.Clock();renderer.setAnimationLoop(()=>{const delta=Math.min(.05,animationClock.getDelta()),time=(performance.now()-introStart)/1000,progress=Math.min(1,time/introDuration),eased=1-Math.pow(1-progress,3);updateTimeOfDay();weatherSystem.update(delta,time);updateFountain(time);if(goal){camera.position.lerp(goal,.07);if(camera.position.distanceTo(goal)<.03)goal=null}if(progress<1){camera.position.lerpVectors(introStartPosition,introEndPosition,eased);controls.target.lerpVectors(introStartTarget,introEndTarget,eased)}else if(!controls.enabled){controls.enabled=true;loading.classList.add('is-hidden')}controls.update();renderer.render(scene,camera)});
