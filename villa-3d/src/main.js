...
const introDuration=2.8; let introActive=true; const introStartPosition=camera.position.clone().multiplyScalar(1.42); const introEndPosition=camera.position.clone(); const introStartTarget=controls.target.clone().add(new THREE.Vector3(0,1.15,0)); const introEndTarget=controls.target.clone(); camera.position.copy(introStartPosition); controls.target.copy(introStartTarget); controls.enabled=false; controls.update();
const clock=new THREE.Clock();
renderer.setAnimationLoop(()=>{ const time=clock.getElapsedTime(), weather=livingEnvironment.update(time), now=new Date(), daylight=setLightingFromLocalTime(scene,lights,now,weather); publishAmbience(weather,daylight,now); warmLight.intensity=1.7+(1-daylight)*4.4; interiorLights.forEach(light=>{ light.intensity=light.userData.interiorBaseIntensity*(.62+(1-daylight)*.72); }); if(introActive){ const progress=Math.min(1,time/introDuration), eased=1-Math.pow(1-progress,3); camera.position.lerpVectors(introStartPosition,introEndPosition,eased); controls.target.lerpVectors(introStartTarget,introEndTarget,eased); controls.update(); if(progress>=1){ introActive=false; controls.enabled=true; ui.hideLoading(); } } else { controls.update(); } cutaway.update(); renderer.render(scene,camera); });
