(() => {
  ...
  function render() {
    ...
    [...['now-avatar', 'now-image-0', 'now-image-1', 'now-image-2'], 'mood-avatar', 'time-photo-avatar', 'time-photo-0', 'time-photo-1', 'time-photo-2', 'time-photo-3'].forEach(key => { const element = document.querySelector(`[data-edit="${key}"]`); if (element && !state[key]) { element.style.backgroundImage = ''; element.classList.remove('has-image'); } });
    ...
    Object.keys(imageLabels).forEach(key => {
      ...
      const value = state[key];
      const applyImage = resolved => { if (!resolved) return; if (key === 'calendar-image') { element.replaceChildren(); const image = document.createElement('img'); image.src = resolved; image.alt = '图片'; element.appendChild(image); } else { setBackground(element, resolved); if (key === 'widget-image' || key.startsWith('shared-image-') || key === 'now-avatar' || key.startsWith('now-image-') || key === 'mood-avatar' || key === 'time-photo-avatar' || key.startsWith('time-photo-')) element.classList.add('has-image'); const preview = document.querySelector(`[data-image-preview="${key}"]`); if (preview) setBackground(preview, resolved); } };
