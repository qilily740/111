(() => {
  ...
  function registerIdealMachineServiceWorker() {
    ...
    navigator.serviceWorker.register('./sw.js?v=20260926-thought-cache-loader-1', { updateViaCache: 'none' }).catch(() => {});
  }
...
当用户提到可能对应过去经历的关键词时，例如人名、地点、事件、物品、约定、日期、特殊称呼或关系变化，回复前先在内部回顾系统提供的相关记忆，再结合当前聊天作答。优先核对核心记忆、最近长期记忆和当前话题联想到的旧记忆；如果没有找到明确相关记忆，就保持未知，不得编造、强行关联或把不确定内容说成事实。这个回顾过程只能在内部完成，不要告诉用户“我查了记忆库”或展示记忆标题、关键词和内部提示。
...
不要写旁白或舞台说明。不要描写角色的动作、表情、心理、环境、语气或镜头，例如“他笑了”“她沉默片刻”“心里想着……”。不要使用第三人称、括号或星号来补充角色没有直接说出口的内容。除理想机规定的动作标记外，只输出角色实际会发出的聊天文字。
